...
*/
namespace com\bydan\contabilidad\cuentaspagar\cuenta_pagar_detalle\business\entity;
...
class cuenta_pagar_detalle extends GeneralEntity {

	/*GENERAL*/
	public static string $class='cuenta_pagar_detalle';
	
	/*AUXILIARES*/
	public ?cuenta_pagar_detalle $cuenta_pagar_detalle_Original=null;	
	public ?GeneralEntity $cuenta_pagar_detalle_Additional=null;
	public array $map_cuenta_pagar_detalle=array();	
	...
	function __construct () {
		...
		$this->cuenta_pagar_detalle_Original=$this;
		*/
		...
		$this->cuenta_pagar_detalle_Additional=new cuenta_pagar_detalle_Additional();*/
    } 
	...
	public function inicializarMap_cuenta_pagar_detalle() {
		$this->map_cuenta_pagar_detalle = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->id_cuenta_pagar!==$newid_cuenta_pagar) {
				if($newid_cuenta_pagar===null && $newid_cuenta_pagar!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_cuenta_pagar');
				}

				if($newid_cuenta_pagar!==null && filter_var($newid_cuenta_pagar,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_cuenta_pagar='.$newid_cuenta_pagar);
				}
		...
		try {
			if($this->numero!==$newnumero) {
				if($newnumero===null && $newnumero!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna numero');
				}

				if(strlen($newnumero)>12) {
					throw new Exception('cuenta_pagar_detalle:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=12 en columna numero');
				}

				if(filter_var($newnumero,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('cuenta_pagar_detalle:Formato incorrecto en columna numero='.$newnumero);
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('cuenta_pagar_detalle:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->fecha_vence!==$newfecha_vence) {
				if($newfecha_vence===null && $newfecha_vence!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna fecha_vence');
				}
				...
				if($newfecha_vence!==null && filter_var($newfecha_vence,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('cuenta_pagar_detalle:No es fecha - fecha_vence='.$newfecha_vence);
				}
		...
		try {
			if($this->referencia!==$newreferencia) {
				if($newreferencia===null && $newreferencia!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna referencia');
				}

				if(strlen($newreferencia)>25) {
					throw new Exception('cuenta_pagar_detalle:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=25 en columna referencia');
				}
				...
				if(filter_var($newreferencia,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('cuenta_pagar_detalle:Formato incorrecto en columna referencia='.$newreferencia);
				}
		...
		try {
			if($this->monto!==$newmonto) {
				if($newmonto===null && $newmonto!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna monto');
				}

				if($newmonto!=0) {
					if($newmonto!==null && filter_var($newmonto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('cuenta_pagar_detalle:No es numero decimal - monto='.$newmonto);
					}
				}
		...
		try {
			if($this->debito!==$newdebito) {
				if($newdebito===null && $newdebito!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna debito');
				}

				if($newdebito!=0) {
					if($newdebito!==null && filter_var($newdebito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('cuenta_pagar_detalle:No es numero decimal - debito='.$newdebito);
					}
				}
		...
		try {
			if($this->credito!==$newcredito) {
				if($newcredito===null && $newcredito!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna credito');
				}

				if($newcredito!=0) {
					if($newcredito!==null && filter_var($newcredito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('cuenta_pagar_detalle:No es numero decimal - credito='.$newcredito);
					}
				}
	...
	public function setdescripcion(?string $newdescripcion) {
		if($this->descripcion!==$newdescripcion) {
				...
				if(strlen($newdescripcion)>1000) {
					try {
						throw new Exception('cuenta_pagar_detalle:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=descripcion en columna descripcion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newdescripcion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('cuenta_pagar_detalle:Formato incorrecto en la columna descripcion='.$newdescripcion);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->id_estado!==$newid_estado) {
				if($newid_estado===null && $newid_estado!='') {
					throw new Exception('cuenta_pagar_detalle:Valor nulo no permitido en columna id_estado');
				}

				if($newid_estado!==null && filter_var($newid_estado,FILTER_VALIDATE_INT)===false) {
					throw new Exception('cuenta_pagar_detalle:No es numero entero - id_estado='.$newid_estado);
				}
	...
	/*AUXILIARES*/
	public function setMap_cuenta_pagar_detalleValue(string $sKey,$oValue) {
		$this->map_cuenta_pagar_detalle[$sKey]=$oValue;
	}
	
	public function getMap_cuenta_pagar_detalleValue(string $sKey) {
		return $this->map_cuenta_pagar_detalle[$sKey];
	}
	
	public function getcuenta_pagar_detalle_Original() : ?cuenta_pagar_detalle {
		return $this->cuenta_pagar_detalle_Original;
	}
	
	public function setcuenta_pagar_detalle_Original(cuenta_pagar_detalle $cuenta_pagar_detalle) {
		try {
			$this->cuenta_pagar_detalle_Original=$cuenta_pagar_detalle;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_cuenta_pagar_detalle() : array {
		return $this->map_cuenta_pagar_detalle;
	}

	public function setMap_cuenta_pagar_detalle(array $map_cuenta_pagar_detalle) {
		$this->map_cuenta_pagar_detalle = $map_cuenta_pagar_detalle;
	}

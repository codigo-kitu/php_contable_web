		...
		<?php if($IS_DEVELOPING) { ?>
			...
			<th class="cabecera_titulo_tabla" >
				<pre class="cabecera_texto_titulo_tabla">Sucursal<a onclick="jQuery('#form-id_sucursal_img').trigger('click' );"><img id="form$strSufijo-id_sucursal_img2" name="form$strSufijo-id_sucursal_img2" class="imagen_actualizar" title="ACTUALIZAR DATOS RELACIONADOS" src="<?php echo($PATH_IMAGEN) ?>Imagenes/actualizardatos.gif" alt="Actualizar Datos" border="" height="15" width="15"></a><a onclick="documento_cuenta_pagar_webcontrol1.abrirBusquedaParasucursal('id_sucursal');"><img id="form$strSufijo-id_sucursal_img_busqueda2" name="form$strSufijo-id_sucursal_img_busqueda2" title="Buscar Sucursal" style="width: 15px; height: 15px;text-align: center; visibility:visible;" src="<?php echo($PATH_IMAGEN) ?>Imagenes/buscar.gif" width="15" height="15"/></a></pre>
			</th>
		<?php } ?>
			...
			<?php foreach ($documento_cuenta_pagarsLocal as $documento_cuenta_pagar) { ?>
				...
				<td class="elementotabla col_id" >
					...
					<table>
						<tr>
							...
							<td>
								<img class="imgseleccionardocumento_cuenta_pagar" idactualdocumento_cuenta_pagar="<?php echo($documento_cuenta_pagar->id) ?>"  funcionjsactualdocumento_cuenta_pagar="'.str_replace('TO_REPLACE',$documento_cuenta_pagar->id.',\''.documento_cuenta_pagar_util::getdocumento_cuenta_pagarDescripcion($documento_cuenta_pagar).'\'',$this->strFuncionJs).'" title="SELECCIONAR Documentos Cuentas por Pagar ACTUAL" src="<?php echo($PATH_IMAGEN) ?>/Imagenes/seleccionar.gif" alt="Seleccionar" border="0" height="15" width="15">
							</td>
						</tr>
					</table>
					</a>
				...
				<td class="elementotabla columna_tabla_sel col_id" >
					<table>
						<tr>
							<td>
								<input id="t-id_<?php echo($i) ?>" name="t-id_<?php echo($i) ?>" type="checkbox" class="chkb_id" title="SELECCIONAR Documentos Cuentas por Pagar ACTUAL" value="<?php echo($documento_cuenta_pagar->id) ?>"></input>
							</td>
							<td>
								<input id="t-cel_<?php echo($i) ?>_0" name="t-cel_<?php echo($i) ?>_0" type="text" maxlength="25" value="<?php echo($documento_cuenta_pagar->id) ?>" style="width:50px" readonly></input>
							</td>
						</tr>
					</table>
				</td>
					...
					<td class="elementotabla col_created_at"  style="display:none"> 
						<?php echo( $documento_cuenta_pagar->updated_at ) ?>
					</td>
				...
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($documento_cuenta_pagar->id_empresa_Descripcion) ?></td>
				<?php } ?>
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($documento_cuenta_pagar->id_sucursal_Descripcion) ?></td>
				<?php } ?>
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($documento_cuenta_pagar->id_ejercicio_Descripcion) ?></td>
				<?php } ?>
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($documento_cuenta_pagar->id_periodo_Descripcion) ?></td>
				<?php } ?>
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($documento_cuenta_pagar->id_usuario_Descripcion) ?></td>
				<?php } ?>
				
					<td class="elementotabla col_numero" > 
						<?php echo( $documento_cuenta_pagar->numero ) ?>
					</td>
				<td class="elementotabla col_id_proveedor" > <?php echo($documento_cuenta_pagar->id_proveedor_Descripcion) ?></td>
				
					<td class="elementotabla col_tipo" > 
						<?php echo( $documento_cuenta_pagar->tipo ) ?>
					</td>
				
					<td class="elementotabla col_fecha_emision" > 
						<?php echo( $documento_cuenta_pagar->fecha_emision ) ?>
					</td>
				
					<td class="elementotabla col_descripcion" > 
						<?php echo( $documento_cuenta_pagar->descripcion ) ?>
					</td>
				
					<td class="elementotabla col_monto" > 
						<?php echo( $documento_cuenta_pagar->monto ) ?>
					</td>
				<td class="elementotabla col_id_forma_pago_proveedor" > <?php echo($documento_cuenta_pagar->id_forma_pago_proveedor_Descripcion) ?></td>
				<td class="elementotabla col_id_cuenta_corriente" > <?php echo($documento_cuenta_pagar->id_cuenta_corriente_Descripcion) ?></td>

		...
		<?php if($IS_DEVELOPING) { ?>
			...
			<th class="cabecera_titulo_tabla" >
				<pre class="cabecera_texto_titulo_tabla">Sucursal<a onclick="jQuery('#form-id_sucursal_img').trigger('click' );"><img id="form$strSufijo-id_sucursal_img2" name="form$strSufijo-id_sucursal_img2" class="imagen_actualizar" title="ACTUALIZAR DATOS RELACIONADOS" src="<?php echo($PATH_IMAGEN) ?>Imagenes/actualizardatos.gif" alt="Actualizar Datos" border="" height="15" width="15"></a><a onclick="bodega_webcontrol1.abrirBusquedaParasucursal('id_sucursal');"><img id="form$strSufijo-id_sucursal_img_busqueda2" name="form$strSufijo-id_sucursal_img_busqueda2" title="Buscar Sucursal" style="width: 15px; height: 15px;text-align: center; visibility:visible;" src="<?php echo($PATH_IMAGEN) ?>Imagenes/buscar.gif" width="15" height="15"/></a></pre>
			</th>
		<?php } ?>
			...
			<?php foreach ($bodegasLocal as $bodega) { ?>
				...
				<td class="elementotabla col_id" >
					...
					<table>
						<tr>
							...
							<td>
								<img class="imgseleccionarbodega" idactualbodega="<?php echo($bodega->id) ?>"  funcionjsactualbodega="'.str_replace('TO_REPLACE',$bodega->id.',\''.bodega_util::getbodegaDescripcion($bodega).'\'',$this->strFuncionJs).'" title="SELECCIONAR Bodega ACTUAL" src="<?php echo($PATH_IMAGEN) ?>/Imagenes/seleccionar.gif" alt="Seleccionar" border="0" height="15" width="15">
							</td>
						</tr>
					</table>
					</a>
				...
				<td class="elementotabla columna_tabla_sel col_id" >
					<table>
						<tr>
							<td>
								<input id="t-id_<?php echo($i) ?>" name="t-id_<?php echo($i) ?>" type="checkbox" class="chkb_id" title="SELECCIONAR Bodega ACTUAL" value="<?php echo($bodega->id) ?>"></input>
							</td>
							<td>
								<input id="t-cel_<?php echo($i) ?>_0" name="t-cel_<?php echo($i) ?>_0" type="text" maxlength="25" value="<?php echo($bodega->id) ?>" style="width:50px" readonly></input>
							</td>
						</tr>
					</table>
				</td>
				...
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($bodega->id_empresa_Descripcion) ?></td>
				<?php } ?>
				<?php if($IS_DEVELOPING) { ?>
				<td> <?php echo($bodega->id_sucursal_Descripcion) ?></td>
				<?php } ?>
				
					<td class="elementotabla col_codigo" > 
						<?php echo( $bodega->codigo ) ?>
					</td>
					...
					<td class="elementotabla col_numero_productos" > 
						<?php echo( $bodega->numero_productos ) ?>
					</td>

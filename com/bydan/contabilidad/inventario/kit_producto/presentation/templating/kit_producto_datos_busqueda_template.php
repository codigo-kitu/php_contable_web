			...
			<?php foreach ($kit_productosLocal as $kit_producto) { ?>
				...
				<td class="elementotabla col_id" >
					...
					<table>
						<tr>
							...
							<td>
								<img class="imgseleccionarkit_producto" idactualkit_producto="<?php echo($kit_producto->id) ?>"  funcionjsactualkit_producto="'.str_replace('TO_REPLACE',$kit_producto->id.',\''.kit_producto_util::getkit_productoDescripcion($kit_producto).'\'',$this->strFuncionJs).'" title="SELECCIONAR Kits Producto ACTUAL" src="<?php echo($PATH_IMAGEN) ?>/Imagenes/seleccionar.gif" alt="Seleccionar" border="0" height="15" width="15">
							</td>
						</tr>
					</table>
					</a>
				...
				<td class="elementotabla columna_tabla_sel col_id" >
					<table>
						<tr>
							<td>
								<input id="t-id_<?php echo($i) ?>" name="t-id_<?php echo($i) ?>" type="checkbox" class="chkb_id" title="SELECCIONAR Kits Producto ACTUAL" value="<?php echo($kit_producto->id) ?>"></input>
							</td>
							<td>
								<input id="t-cel_<?php echo($i) ?>_0" name="t-cel_<?php echo($i) ?>_0" type="text" maxlength="25" value="<?php echo($kit_producto->id) ?>" style="width:50px" readonly></input>
							</td>
						</tr>
					</table>
				</td>
					...
					<td class="elementotabla col_created_at"  style="display:none"> 
						<?php echo( $kit_producto->updated_at ) ?>
					</td>
					...
					<td class="elementotabla col_id_padre" > 
						<?php echo( $kit_producto->id_padre ) ?>
					</td>
				
					<td class="elementotabla col_id_hijo" > 
						<?php echo( $kit_producto->id_hijo ) ?>
					</td>
				
					<td class="elementotabla col_cantidad" > 
						<?php echo( $kit_producto->cantidad ) ?>
					</td>

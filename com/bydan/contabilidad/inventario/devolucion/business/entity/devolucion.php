...
*/
namespace com\bydan\contabilidad\inventario\devolucion\business\entity;
...
class devolucion extends GeneralEntity {

	/*GENERAL*/
	public static string $class='devolucion';
	
	/*AUXILIARES*/
	public ?devolucion $devolucion_Original=null;	
	public ?GeneralEntity $devolucion_Additional=null;
	public array $map_devolucion=array();	
	...
	public array $devoluciondetalles = array();
	...
	function __construct () {
		...
		*/
		...
		$this->devoluciondetalles=array();
		
		/*AUTOREFERENCIA INFINITA TALVEZ
		$this->devolucion_Additional=new devolucion_Additional();*/
    } 
	...
	public function inicializarMap_devolucion() {
		$this->map_devolucion = array();
	}
	...
	public function  getenvia() : ?string {
		return $this->envia;
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->numero!==$newnumero) {
				if($newnumero===null && $newnumero!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna numero');
				}

				if(strlen($newnumero)>10) {
					throw new Exception('devolucion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=10 en columna numero');
				}

				if(filter_var($newnumero,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('devolucion:Formato incorrecto en columna numero='.$newnumero);
				}
		...
		try {
			if($this->id_proveedor!==$newid_proveedor) {
				if($newid_proveedor===null && $newid_proveedor!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_proveedor');
				}

				if($newid_proveedor!==null && filter_var($newid_proveedor,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_proveedor='.$newid_proveedor);
				}
		...
		try {
			if($this->id_vendedor!==$newid_vendedor) {
				if($newid_vendedor===null && $newid_vendedor!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_vendedor');
				}

				if($newid_vendedor!==null && filter_var($newid_vendedor,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_vendedor='.$newid_vendedor);
				}
		...
		try {
			if($this->ruc!==$newruc) {
				if($newruc===null && $newruc!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna ruc');
				}

				if(strlen($newruc)>20) {
					throw new Exception('devolucion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=20 en columna ruc');
				}

				if(filter_var($newruc,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('devolucion:Formato incorrecto en columna ruc='.$newruc);
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('devolucion:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->id_termino_pago_proveedor!==$newid_termino_pago_proveedor) {
				if($newid_termino_pago_proveedor===null && $newid_termino_pago_proveedor!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_termino_pago_proveedor');
				}

				if($newid_termino_pago_proveedor!==null && filter_var($newid_termino_pago_proveedor,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_termino_pago_proveedor='.$newid_termino_pago_proveedor);
				}
		...
		try {
			if($this->fecha_vence!==$newfecha_vence) {
				if($newfecha_vence===null && $newfecha_vence!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna fecha_vence');
				}
				...
				if($newfecha_vence!==null && filter_var($newfecha_vence,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('devolucion:No es fecha - fecha_vence='.$newfecha_vence);
				}
		...
		try {
			if($this->cotizacion!==$newcotizacion) {
				if($newcotizacion===null && $newcotizacion!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna cotizacion');
				}

				if($newcotizacion!=0) {
					if($newcotizacion!==null && filter_var($newcotizacion,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - cotizacion='.$newcotizacion);
					}
				}
		...
		try {
			if($this->id_moneda!==$newid_moneda) {
				if($newid_moneda===null && $newid_moneda!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_moneda');
				}

				if($newid_moneda!==null && filter_var($newid_moneda,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_moneda='.$newid_moneda);
				}
		...
		try {
			if($this->id_estado!==$newid_estado) {
				if($newid_estado===null && $newid_estado!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna id_estado');
				}

				if($newid_estado!==null && filter_var($newid_estado,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_estado='.$newid_estado);
				}
	...
	public function setdireccion(?string $newdireccion) {
		if($this->direccion!==$newdireccion) {
				...
				if(strlen($newdireccion)>250) {
					try {
						throw new Exception('devolucion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=direccion en columna direccion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newdireccion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('devolucion:Formato incorrecto en la columna direccion='.$newdireccion);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setenvia(?string $newenvia) {
		if($this->envia!==$newenvia) {

				if(strlen($newenvia)>80) {
					try {
						throw new Exception('devolucion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=envia en columna envia');
					} catch(Exception $e) {
						throw $e;
					}
				}

				if(filter_var($newenvia,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('devolucion:Formato incorrecto en la columna envia='.$newenvia);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setobservacion(?string $newobservacion) {
		if($this->observacion!==$newobservacion) {
				...
				if(strlen($newobservacion)>100) {
					try {
						throw new Exception('devolucion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=observacion en columna observacion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newobservacion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('devolucion:Formato incorrecto en la columna observacion='.$newobservacion);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->impuesto_en_precio!==$newimpuesto_en_precio) {
				if($newimpuesto_en_precio===null && $newimpuesto_en_precio!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna impuesto_en_precio');
				}
				...
				if($newimpuesto_en_precio!==null && filter_var($newimpuesto_en_precio,FILTER_VALIDATE_BOOLEAN)===false && $newimpuesto_en_precio!==0 && $newimpuesto_en_precio!==false) {
					throw new Exception('devolucion:No es valor booleano - impuesto_en_precio='.$newimpuesto_en_precio);
				}
		...
		try {
			if($this->sub_total!==$newsub_total) {
				if($newsub_total===null && $newsub_total!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna sub_total');
				}

				if($newsub_total!=0) {
					if($newsub_total!==null && filter_var($newsub_total,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - sub_total='.$newsub_total);
					}
				}
		...
		try {
			if($this->descuento_monto!==$newdescuento_monto) {
				if($newdescuento_monto===null && $newdescuento_monto!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna descuento_monto');
				}

				if($newdescuento_monto!=0) {
					if($newdescuento_monto!==null && filter_var($newdescuento_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - descuento_monto='.$newdescuento_monto);
					}
				}
		...
		try {
			if($this->descuento_porciento!==$newdescuento_porciento) {
				if($newdescuento_porciento===null && $newdescuento_porciento!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna descuento_porciento');
				}

				if($newdescuento_porciento!=0) {
					if($newdescuento_porciento!==null && filter_var($newdescuento_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - descuento_porciento='.$newdescuento_porciento);
					}
				}
		...
		try {
			if($this->iva_monto!==$newiva_monto) {
				if($newiva_monto===null && $newiva_monto!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna iva_monto');
				}

				if($newiva_monto!=0) {
					if($newiva_monto!==null && filter_var($newiva_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - iva_monto='.$newiva_monto);
					}
				}
		...
		try {
			if($this->iva_porciento!==$newiva_porciento) {
				if($newiva_porciento===null && $newiva_porciento!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna iva_porciento');
				}

				if($newiva_porciento!=0) {
					if($newiva_porciento!==null && filter_var($newiva_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - iva_porciento='.$newiva_porciento);
					}
				}
		...
		try {
			if($this->total!==$newtotal) {
				if($newtotal===null && $newtotal!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna total');
				}

				if($newtotal!=0) {
					if($newtotal!==null && filter_var($newtotal,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - total='.$newtotal);
					}
				}
		...
		try {
			if($this->otro_monto!==$newotro_monto) {
				if($newotro_monto===null && $newotro_monto!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna otro_monto');
				}

				if($newotro_monto!=0) {
					if($newotro_monto!==null && filter_var($newotro_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - otro_monto='.$newotro_monto);
					}
				}
		...
		try {
			if($this->otro_porciento!==$newotro_porciento) {
				if($newotro_porciento===null && $newotro_porciento!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna otro_porciento');
				}

				if($newotro_porciento!=0) {
					if($newotro_porciento!==null && filter_var($newotro_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - otro_porciento='.$newotro_porciento);
					}
				}
	...
	public function setfactura_proveedor(?string $newfactura_proveedor) {
		if($this->factura_proveedor!==$newfactura_proveedor) {
				...
				if(strlen($newfactura_proveedor)>30) {
					try {
						throw new Exception('devolucion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=factura_proveedor en columna factura_proveedor');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newfactura_proveedor,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('devolucion:Formato incorrecto en la columna factura_proveedor='.$newfactura_proveedor);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setpago(?float $newpago)
	{
		try {
			if($this->pago!==$newpago) {
				if($newpago===null && $newpago!='') {
					throw new Exception('devolucion:Valor nulo no permitido en columna pago');
				}

				if($newpago!=0) {
					if($newpago!==null && filter_var($newpago,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('devolucion:No es numero decimal - pago='.$newpago);
					}
				}
	...
	public function setid_asiento(?int $newid_asiento) {
		if($this->id_asiento!==$newid_asiento) {
				...
				if($newid_asiento!==null && filter_var($newid_asiento,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_asiento');
				}
	...
	public function setid_documento_cuenta_pagar(?int $newid_documento_cuenta_pagar) {
		if($this->id_documento_cuenta_pagar!==$newid_documento_cuenta_pagar) {
				...
				if($newid_documento_cuenta_pagar!==null && filter_var($newid_documento_cuenta_pagar,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_documento_cuenta_pagar');
				}
	...
	public function setid_kardex(?int $newid_kardex) {
		if($this->id_kardex!==$newid_kardex) {
				...
				if($newid_kardex!==null && filter_var($newid_kardex,FILTER_VALIDATE_INT)===false) {
					throw new Exception('devolucion:No es numero entero - id_kardex');
				}
	...
	public  function  setdevolucion_detalles(array $devoluciondetalles) {
		try {
	...
	/*AUXILIARES*/
	public function setMap_devolucionValue(string $sKey,$oValue) {
		$this->map_devolucion[$sKey]=$oValue;
	}
	
	public function getMap_devolucionValue(string $sKey) {
		return $this->map_devolucion[$sKey];
	}
	
	public function getdevolucion_Original() : ?devolucion {
		return $this->devolucion_Original;
	}
	
	public function setdevolucion_Original(devolucion $devolucion) {
		try {
			$this->devolucion_Original=$devolucion;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_devolucion() : array {
		return $this->map_devolucion;
	}

	public function setMap_devolucion(array $map_devolucion) {
		$this->map_devolucion = $map_devolucion;
	}

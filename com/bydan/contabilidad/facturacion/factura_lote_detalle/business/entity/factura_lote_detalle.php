...
*/
namespace com\bydan\contabilidad\facturacion\factura_lote_detalle\business\entity;
...
class factura_lote_detalle extends GeneralEntity {

	/*GENERAL*/
	public static string $class='factura_lote_detalle';
	
	/*AUXILIARES*/
	public ?factura_lote_detalle $factura_lote_detalle_Original=null;	
	public ?GeneralEntity $factura_lote_detalle_Additional=null;
	public array $map_factura_lote_detalle=array();	
	...
	function __construct () {
		...
		$this->factura_lote_detalle_Original=$this;
		*/
		...
		$this->factura_lote_detalle_Additional=new factura_lote_detalle_Additional();*/
    } 
	...
	public function inicializarMap_factura_lote_detalle() {
		$this->map_factura_lote_detalle = array();
	}
	...
	public function setid_factura_lote(?int $newid_factura_lote) {
		if($this->id_factura_lote!==$newid_factura_lote) {
				...
				if($newid_factura_lote!==null && filter_var($newid_factura_lote,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura_lote_detalle:No es numero entero - id_factura_lote');
				}
		...
		try {
			if($this->id_bodega!==$newid_bodega) {
				if($newid_bodega===null && $newid_bodega!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna id_bodega');
				}

				if($newid_bodega!==null && filter_var($newid_bodega,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura_lote_detalle:No es numero entero - id_bodega='.$newid_bodega);
				}
		...
		try {
			if($this->id_producto!==$newid_producto) {
				if($newid_producto===null && $newid_producto!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna id_producto');
				}

				if($newid_producto!==null && filter_var($newid_producto,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura_lote_detalle:No es numero entero - id_producto='.$newid_producto);
				}
		...
		try {
			if($this->id_unidad!==$newid_unidad) {
				if($newid_unidad===null && $newid_unidad!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna id_unidad');
				}

				if($newid_unidad!==null && filter_var($newid_unidad,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura_lote_detalle:No es numero entero - id_unidad='.$newid_unidad);
				}
		...
		try {
			if($this->cantidad!==$newcantidad) {
				if($newcantidad===null && $newcantidad!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna cantidad');
				}

				if($newcantidad!=0) {
					if($newcantidad!==null && filter_var($newcantidad,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - cantidad='.$newcantidad);
					}
				}
		...
		try {
			if($this->precio!==$newprecio) {
				if($newprecio===null && $newprecio!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna precio');
				}

				if($newprecio!=0) {
					if($newprecio!==null && filter_var($newprecio,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - precio='.$newprecio);
					}
				}
		...
		try {
			if($this->descuento_porciento!==$newdescuento_porciento) {
				if($newdescuento_porciento===null && $newdescuento_porciento!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna descuento_porciento');
				}

				if($newdescuento_porciento!=0) {
					if($newdescuento_porciento!==null && filter_var($newdescuento_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - descuento_porciento='.$newdescuento_porciento);
					}
				}
		...
		try {
			if($this->descuento_monto!==$newdescuento_monto) {
				if($newdescuento_monto===null && $newdescuento_monto!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna descuento_monto');
				}

				if($newdescuento_monto!=0) {
					if($newdescuento_monto!==null && filter_var($newdescuento_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - descuento_monto='.$newdescuento_monto);
					}
				}
		...
		try {
			if($this->sub_total!==$newsub_total) {
				if($newsub_total===null && $newsub_total!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna sub_total');
				}

				if($newsub_total!=0) {
					if($newsub_total!==null && filter_var($newsub_total,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - sub_total='.$newsub_total);
					}
				}
		...
		try {
			if($this->iva_porciento!==$newiva_porciento) {
				if($newiva_porciento===null && $newiva_porciento!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna iva_porciento');
				}

				if($newiva_porciento!=0) {
					if($newiva_porciento!==null && filter_var($newiva_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - iva_porciento='.$newiva_porciento);
					}
				}
		...
		try {
			if($this->iva_monto!==$newiva_monto) {
				if($newiva_monto===null && $newiva_monto!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna iva_monto');
				}

				if($newiva_monto!=0) {
					if($newiva_monto!==null && filter_var($newiva_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - iva_monto='.$newiva_monto);
					}
				}
		...
		try {
			if($this->total!==$newtotal) {
				if($newtotal===null && $newtotal!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna total');
				}

				if($newtotal!=0) {
					if($newtotal!==null && filter_var($newtotal,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - total='.$newtotal);
					}
				}
		...
		try {
			if($this->recibido!==$newrecibido) {
				if($newrecibido===null && $newrecibido!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna recibido');
				}

				if($newrecibido!=0) {
					if($newrecibido!==null && filter_var($newrecibido,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - recibido='.$newrecibido);
					}
				}
	...
	public function setobservacion(?string $newobservacion) {
		if($this->observacion!==$newobservacion) {
				...
				if(strlen($newobservacion)>1000) {
					try {
						throw new Exception('factura_lote_detalle:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=observacion en columna observacion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newobservacion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('factura_lote_detalle:Formato incorrecto en la columna observacion='.$newobservacion);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->impuesto2_porciento!==$newimpuesto2_porciento) {
				if($newimpuesto2_porciento===null && $newimpuesto2_porciento!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna impuesto2_porciento');
				}

				if($newimpuesto2_porciento!=0) {
					if($newimpuesto2_porciento!==null && filter_var($newimpuesto2_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - impuesto2_porciento='.$newimpuesto2_porciento);
					}
				}
		...
		try {
			if($this->impuesto2_monto!==$newimpuesto2_monto) {
				if($newimpuesto2_monto===null && $newimpuesto2_monto!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna impuesto2_monto');
				}

				if($newimpuesto2_monto!=0) {
					if($newimpuesto2_monto!==null && filter_var($newimpuesto2_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - impuesto2_monto='.$newimpuesto2_monto);
					}
				}
		...
		try {
			if($this->cotizacion!==$newcotizacion) {
				if($newcotizacion===null && $newcotizacion!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna cotizacion');
				}

				if($newcotizacion!=0) {
					if($newcotizacion!==null && filter_var($newcotizacion,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura_lote_detalle:No es numero decimal - cotizacion='.$newcotizacion);
					}
				}
		...
		try {
			if($this->moneda!==$newmoneda) {
				if($newmoneda===null && $newmoneda!='') {
					throw new Exception('factura_lote_detalle:Valor nulo no permitido en columna moneda');
				}

				if($newmoneda!==null && filter_var($newmoneda,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura_lote_detalle:No es numero entero - moneda='.$newmoneda);
				}
	...
	/*AUXILIARES*/
	public function setMap_factura_lote_detalleValue(string $sKey,$oValue) {
		$this->map_factura_lote_detalle[$sKey]=$oValue;
	}
	
	public function getMap_factura_lote_detalleValue(string $sKey) {
		return $this->map_factura_lote_detalle[$sKey];
	}
	
	public function getfactura_lote_detalle_Original() : ?factura_lote_detalle {
		return $this->factura_lote_detalle_Original;
	}
	
	public function setfactura_lote_detalle_Original(factura_lote_detalle $factura_lote_detalle) {
		try {
			$this->factura_lote_detalle_Original=$factura_lote_detalle;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_factura_lote_detalle() : array {
		return $this->map_factura_lote_detalle;
	}

	public function setMap_factura_lote_detalle(array $map_factura_lote_detalle) {
		$this->map_factura_lote_detalle = $map_factura_lote_detalle;
	}

...
*/
namespace com\bydan\contabilidad\facturacion\retencion\business\entity;
...
class retencion extends GeneralEntity {

	/*GENERAL*/
	public static string $class='retencion';
	
	/*AUXILIARES*/
	public ?retencion $retencion_Original=null;	
	public ?GeneralEntity $retencion_Additional=null;
	public array $map_retencion=array();	
	...
	function __construct () {
		...
		*/
		...
		$this->retencion_Additional=new retencion_Additional();*/
    } 
	...
	public function inicializarMap_retencion() {
		$this->map_retencion = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('retencion:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retencion:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->codigo!==$newcodigo) {
				if($newcodigo===null && $newcodigo!='') {
					throw new Exception('retencion:Valor nulo no permitido en columna codigo');
				}

				if(strlen($newcodigo)>5) {
					throw new Exception('retencion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=5 en columna codigo');
				}

				if(filter_var($newcodigo,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('retencion:Formato incorrecto en columna codigo='.$newcodigo);
				}
		...
		try {
			if($this->descripcion!==$newdescripcion) {
				if($newdescripcion===null && $newdescripcion!='') {
					throw new Exception('retencion:Valor nulo no permitido en columna descripcion');
				}

				if(strlen($newdescripcion)>40) {
					throw new Exception('retencion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=40 en columna descripcion');
				}
				...
				if(filter_var($newdescripcion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('retencion:Formato incorrecto en columna descripcion='.$newdescripcion);
				}
		...
		try {
			if($this->valor!==$newvalor) {
				if($newvalor===null && $newvalor!='') {
					throw new Exception('retencion:Valor nulo no permitido en columna valor');
				}

				if($newvalor!=0) {
					if($newvalor!==null && filter_var($newvalor,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('retencion:No es numero decimal - valor='.$newvalor);
					}
				}
		...
		try {
			if($this->valor_base!==$newvalor_base) {
				if($newvalor_base===null && $newvalor_base!='') {
					throw new Exception('retencion:Valor nulo no permitido en columna valor_base');
				}

				if($newvalor_base!=0) {
					if($newvalor_base!==null && filter_var($newvalor_base,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('retencion:No es numero decimal - valor_base='.$newvalor_base);
					}
				}
		...
		try {
			if($this->predeterminado!==$newpredeterminado) {
				if($newpredeterminado===null && $newpredeterminado!='') {
					throw new Exception('retencion:Valor nulo no permitido en columna predeterminado');
				}
				...
				if($newpredeterminado!==null && filter_var($newpredeterminado,FILTER_VALIDATE_BOOLEAN)===false && $newpredeterminado!==0 && $newpredeterminado!==false) {
					throw new Exception('retencion:No es valor booleano - predeterminado='.$newpredeterminado);
				}
	...
	public function setid_cuenta_ventas(?int $newid_cuenta_ventas) {
		if($this->id_cuenta_ventas!==$newid_cuenta_ventas) {
				...
				if($newid_cuenta_ventas!==null && filter_var($newid_cuenta_ventas,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retencion:No es numero entero - id_cuenta_ventas');
				}
	...
	public function setid_cuenta_compras(?int $newid_cuenta_compras) {
		if($this->id_cuenta_compras!==$newid_cuenta_compras) {
				...
				if($newid_cuenta_compras!==null && filter_var($newid_cuenta_compras,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retencion:No es numero entero - id_cuenta_compras');
				}
	...
	public function setcuenta_contable_ventas(?string $newcuenta_contable_ventas) {
		if($this->cuenta_contable_ventas!==$newcuenta_contable_ventas) {
				...
				if(strlen($newcuenta_contable_ventas)>20) {
					try {
						throw new Exception('retencion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=cuenta_contable_ventas en columna cuenta_contable_ventas');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newcuenta_contable_ventas,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('retencion:Formato incorrecto en la columna cuenta_contable_ventas='.$newcuenta_contable_ventas);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setcuenta_contable_compras(?string $newcuenta_contable_compras) {
		if($this->cuenta_contable_compras!==$newcuenta_contable_compras) {
				...
				if(strlen($newcuenta_contable_compras)>20) {
					try {
						throw new Exception('retencion:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=cuenta_contable_compras en columna cuenta_contable_compras');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newcuenta_contable_compras,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('retencion:Formato incorrecto en la columna cuenta_contable_compras='.$newcuenta_contable_compras);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	/*AUXILIARES*/
	public function setMap_retencionValue(string $sKey,$oValue) {
		$this->map_retencion[$sKey]=$oValue;
	}
	
	public function getMap_retencionValue(string $sKey) {
		return $this->map_retencion[$sKey];
	}
	
	public function getretencion_Original() : ?retencion {
		return $this->retencion_Original;
	}
	
	public function setretencion_Original(retencion $retencion) {
		try {
			$this->retencion_Original=$retencion;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_retencion() : array {
		return $this->map_retencion;
	}

	public function setMap_retencion(array $map_retencion) {
		$this->map_retencion = $map_retencion;
	}

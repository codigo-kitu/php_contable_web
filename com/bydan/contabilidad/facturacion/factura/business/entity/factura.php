...
*/
namespace com\bydan\contabilidad\facturacion\factura\business\entity;
...
class factura extends GeneralEntity {

	/*GENERAL*/
	public static string $class='factura';
	
	/*AUXILIARES*/
	public ?factura $factura_Original=null;	
	public ?GeneralEntity $factura_Additional=null;
	public array $map_factura=array();	
	...
	public array $facturadetalles = array();
	...
	function __construct () {
		...
		*/
		...
		$this->facturadetalles=array();
		
		/*AUTOREFERENCIA INFINITA TALVEZ
		$this->factura_Additional=new factura_Additional();*/
    } 
	...
	public function inicializarMap_factura() {
		$this->map_factura = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->numero!==$newnumero) {
				if($newnumero===null && $newnumero!='') {
					throw new Exception('factura:Valor nulo no permitido en columna numero');
				}

				if(strlen($newnumero)>10) {
					throw new Exception('factura:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=10 en columna numero');
				}

				if(filter_var($newnumero,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('factura:Formato incorrecto en columna numero='.$newnumero);
				}
		...
		try {
			if($this->id_cliente!==$newid_cliente) {
				if($newid_cliente===null && $newid_cliente!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_cliente');
				}

				if($newid_cliente!==null && filter_var($newid_cliente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_cliente='.$newid_cliente);
				}
		...
		try {
			if($this->ruc!==$newruc) {
				if($newruc===null && $newruc!='') {
					throw new Exception('factura:Valor nulo no permitido en columna ruc');
				}

				if(strlen($newruc)>20) {
					throw new Exception('factura:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=20 en columna ruc');
				}

				if(filter_var($newruc,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('factura:Formato incorrecto en columna ruc='.$newruc);
				}
	...
	public function setreferencia_cliente(?string $newreferencia_cliente) {
		if($this->referencia_cliente!==$newreferencia_cliente) {
				...
				if(strlen($newreferencia_cliente)>20) {
					try {
						throw new Exception('factura:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=referencia_cliente en columna referencia_cliente');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newreferencia_cliente,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('factura:Formato incorrecto en la columna referencia_cliente='.$newreferencia_cliente);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('factura:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('factura:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->id_moneda!==$newid_moneda) {
				if($newid_moneda===null && $newid_moneda!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_moneda');
				}

				if($newid_moneda!==null && filter_var($newid_moneda,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_moneda='.$newid_moneda);
				}
		...
		try {
			if($this->id_vendedor!==$newid_vendedor) {
				if($newid_vendedor===null && $newid_vendedor!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_vendedor');
				}

				if($newid_vendedor!==null && filter_var($newid_vendedor,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_vendedor='.$newid_vendedor);
				}
		...
		try {
			if($this->id_termino_pago_cliente!==$newid_termino_pago_cliente) {
				if($newid_termino_pago_cliente===null && $newid_termino_pago_cliente!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_termino_pago_cliente');
				}

				if($newid_termino_pago_cliente!==null && filter_var($newid_termino_pago_cliente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_termino_pago_cliente='.$newid_termino_pago_cliente);
				}
		...
		try {
			if($this->fecha_vence!==$newfecha_vence) {
				if($newfecha_vence===null && $newfecha_vence!='') {
					throw new Exception('factura:Valor nulo no permitido en columna fecha_vence');
				}
				...
				if($newfecha_vence!==null && filter_var($newfecha_vence,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('factura:No es fecha - fecha_vence='.$newfecha_vence);
				}
		...
		try {
			if($this->id_estado!==$newid_estado) {
				if($newid_estado===null && $newid_estado!='') {
					throw new Exception('factura:Valor nulo no permitido en columna id_estado');
				}

				if($newid_estado!==null && filter_var($newid_estado,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_estado='.$newid_estado);
				}
	...
	public function setdireccion(?string $newdireccion) {
		if($this->direccion!==$newdireccion) {
				...
				if(strlen($newdireccion)>250) {
					try {
						throw new Exception('factura:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=direccion en columna direccion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newdireccion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('factura:Formato incorrecto en la columna direccion='.$newdireccion);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setenviar_a(?string $newenviar_a) {
		if($this->enviar_a!==$newenviar_a) {
				...
				if(strlen($newenviar_a)>250) {
					try {
						throw new Exception('factura:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=enviar_a en columna enviar_a');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newenviar_a,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('factura:Formato incorrecto en la columna enviar_a='.$newenviar_a);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setobservacion(?string $newobservacion) {
		if($this->observacion!==$newobservacion) {
				...
				if(strlen($newobservacion)>200) {
					try {
						throw new Exception('factura:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=observacion en columna observacion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newobservacion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('factura:Formato incorrecto en la columna observacion='.$newobservacion);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->impuesto_en_precio!==$newimpuesto_en_precio) {
				if($newimpuesto_en_precio===null && $newimpuesto_en_precio!='') {
					throw new Exception('factura:Valor nulo no permitido en columna impuesto_en_precio');
				}
				...
				if($newimpuesto_en_precio!==null && filter_var($newimpuesto_en_precio,FILTER_VALIDATE_BOOLEAN)===false && $newimpuesto_en_precio!==0 && $newimpuesto_en_precio!==false) {
					throw new Exception('factura:No es valor booleano - impuesto_en_precio='.$newimpuesto_en_precio);
				}
		...
		try {
			if($this->sub_total!==$newsub_total) {
				if($newsub_total===null && $newsub_total!='') {
					throw new Exception('factura:Valor nulo no permitido en columna sub_total');
				}

				if($newsub_total!=0) {
					if($newsub_total!==null && filter_var($newsub_total,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - sub_total='.$newsub_total);
					}
				}
		...
		try {
			if($this->descuento_monto!==$newdescuento_monto) {
				if($newdescuento_monto===null && $newdescuento_monto!='') {
					throw new Exception('factura:Valor nulo no permitido en columna descuento_monto');
				}

				if($newdescuento_monto!=0) {
					if($newdescuento_monto!==null && filter_var($newdescuento_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - descuento_monto='.$newdescuento_monto);
					}
				}
		...
		try {
			if($this->descuento_porciento!==$newdescuento_porciento) {
				if($newdescuento_porciento===null && $newdescuento_porciento!='') {
					throw new Exception('factura:Valor nulo no permitido en columna descuento_porciento');
				}

				if($newdescuento_porciento!=0) {
					if($newdescuento_porciento!==null && filter_var($newdescuento_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - descuento_porciento='.$newdescuento_porciento);
					}
				}
		...
		try {
			if($this->iva_monto!==$newiva_monto) {
				if($newiva_monto===null && $newiva_monto!='') {
					throw new Exception('factura:Valor nulo no permitido en columna iva_monto');
				}

				if($newiva_monto!=0) {
					if($newiva_monto!==null && filter_var($newiva_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - iva_monto='.$newiva_monto);
					}
				}
		...
		try {
			if($this->iva_porciento!==$newiva_porciento) {
				if($newiva_porciento===null && $newiva_porciento!='') {
					throw new Exception('factura:Valor nulo no permitido en columna iva_porciento');
				}

				if($newiva_porciento!=0) {
					if($newiva_porciento!==null && filter_var($newiva_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - iva_porciento='.$newiva_porciento);
					}
				}
		...
		try {
			if($this->retencion_fuente_monto!==$newretencion_fuente_monto) {
				if($newretencion_fuente_monto===null && $newretencion_fuente_monto!='') {
					throw new Exception('factura:Valor nulo no permitido en columna retencion_fuente_monto');
				}
				...
				if($newretencion_fuente_monto!=0) {
					if($newretencion_fuente_monto!==null && filter_var($newretencion_fuente_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - retencion_fuente_monto='.$newretencion_fuente_monto);
					}
				}
		...
		try {
			if($this->retencion_fuente_porciento!==$newretencion_fuente_porciento) {
				if($newretencion_fuente_porciento===null && $newretencion_fuente_porciento!='') {
					throw new Exception('factura:Valor nulo no permitido en columna retencion_fuente_porciento');
				}
				...
				if($newretencion_fuente_porciento!=0) {
					if($newretencion_fuente_porciento!==null && filter_var($newretencion_fuente_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - retencion_fuente_porciento='.$newretencion_fuente_porciento);
					}
				}
		...
		try {
			if($this->retencion_iva_monto!==$newretencion_iva_monto) {
				if($newretencion_iva_monto===null && $newretencion_iva_monto!='') {
					throw new Exception('factura:Valor nulo no permitido en columna retencion_iva_monto');
				}

				if($newretencion_iva_monto!=0) {
					if($newretencion_iva_monto!==null && filter_var($newretencion_iva_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - retencion_iva_monto='.$newretencion_iva_monto);
					}
				}
		...
		try {
			if($this->retencion_iva_porciento!==$newretencion_iva_porciento) {
				if($newretencion_iva_porciento===null && $newretencion_iva_porciento!='') {
					throw new Exception('factura:Valor nulo no permitido en columna retencion_iva_porciento');
				}
				...
				if($newretencion_iva_porciento!=0) {
					if($newretencion_iva_porciento!==null && filter_var($newretencion_iva_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - retencion_iva_porciento='.$newretencion_iva_porciento);
					}
				}
		...
		try {
			if($this->total!==$newtotal) {
				if($newtotal===null && $newtotal!='') {
					throw new Exception('factura:Valor nulo no permitido en columna total');
				}

				if($newtotal!=0) {
					if($newtotal!==null && filter_var($newtotal,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - total='.$newtotal);
					}
				}
		...
		try {
			if($this->hora!==$newhora) {
				if($newhora===null && $newhora!='') {
					throw new Exception('factura:Valor nulo no permitido en columna hora');
				}

				if($newhora!==null && filter_var($newhora,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('factura:No es fecha - hora='.$newhora);
				}
		...
		try {
			if($this->cotizacion!==$newcotizacion) {
				if($newcotizacion===null && $newcotizacion!='') {
					throw new Exception('factura:Valor nulo no permitido en columna cotizacion');
				}

				if($newcotizacion!=0) {
					if($newcotizacion!==null && filter_var($newcotizacion,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - cotizacion='.$newcotizacion);
					}
				}
		...
		try {
			if($this->otro_monto!==$newotro_monto) {
				if($newotro_monto===null && $newotro_monto!='') {
					throw new Exception('factura:Valor nulo no permitido en columna otro_monto');
				}

				if($newotro_monto!=0) {
					if($newotro_monto!==null && filter_var($newotro_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - otro_monto='.$newotro_monto);
					}
				}
		...
		try {
			if($this->otro_porciento!==$newotro_porciento) {
				if($newotro_porciento===null && $newotro_porciento!='') {
					throw new Exception('factura:Valor nulo no permitido en columna otro_porciento');
				}

				if($newotro_porciento!=0) {
					if($newotro_porciento!==null && filter_var($newotro_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - otro_porciento='.$newotro_porciento);
					}
				}
		...
		try {
			if($this->retencion_ica_porciento!==$newretencion_ica_porciento) {
				if($newretencion_ica_porciento===null && $newretencion_ica_porciento!='') {
					throw new Exception('factura:Valor nulo no permitido en columna retencion_ica_porciento');
				}
				...
				if($newretencion_ica_porciento!=0) {
					if($newretencion_ica_porciento!==null && filter_var($newretencion_ica_porciento,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - retencion_ica_porciento='.$newretencion_ica_porciento);
					}
				}
		...
		try {
			if($this->retencion_ica_monto!==$newretencion_ica_monto) {
				if($newretencion_ica_monto===null && $newretencion_ica_monto!='') {
					throw new Exception('factura:Valor nulo no permitido en columna retencion_ica_monto');
				}

				if($newretencion_ica_monto!=0) {
					if($newretencion_ica_monto!==null && filter_var($newretencion_ica_monto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('factura:No es numero decimal - retencion_ica_monto='.$newretencion_ica_monto);
					}
				}
	...
	public function setid_asiento(?int $newid_asiento) {
		if($this->id_asiento!==$newid_asiento) {
				...
				if($newid_asiento!==null && filter_var($newid_asiento,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_asiento');
				}
	...
	public function setid_documento_cuenta_cobrar(?int $newid_documento_cuenta_cobrar) {
		if($this->id_documento_cuenta_cobrar!==$newid_documento_cuenta_cobrar) {
				...
				if($newid_documento_cuenta_cobrar!==null && filter_var($newid_documento_cuenta_cobrar,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_documento_cuenta_cobrar');
				}
	...
	public function setid_kardex(?int $newid_kardex) {
		if($this->id_kardex!==$newid_kardex) {
				...
				if($newid_kardex!==null && filter_var($newid_kardex,FILTER_VALIDATE_INT)===false) {
					throw new Exception('factura:No es numero entero - id_kardex');
				}
	...
	public  function  setfactura_detalles(array $facturadetalles) {
		try {
	...
	/*AUXILIARES*/
	public function setMap_facturaValue(string $sKey,$oValue) {
		$this->map_factura[$sKey]=$oValue;
	}
	
	public function getMap_facturaValue(string $sKey) {
		return $this->map_factura[$sKey];
	}
	
	public function getfactura_Original() : ?factura {
		return $this->factura_Original;
	}
	
	public function setfactura_Original(factura $factura) {
		try {
			$this->factura_Original=$factura;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_factura() : array {
		return $this->map_factura;
	}

	public function setMap_factura(array $map_factura) {
		$this->map_factura = $map_factura;
	}

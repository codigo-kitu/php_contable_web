			...
			<?php foreach ($estimado_detallesLocal as $estimado_detalle) { ?>
				...
				<td class="elementotabla col_id" >
					...
					<table>
						<tr>
							...
							<td>
								<img class="imgseleccionarestimado_detalle" idactualestimado_detalle="<?php echo($estimado_detalle->id) ?>"  funcionjsactualestimado_detalle="'.str_replace('TO_REPLACE',$estimado_detalle->id.',\''.estimado_detalle_util::getestimado_detalleDescripcion($estimado_detalle).'\'',$this->strFuncionJs).'" title="SELECCIONAR Estimado Detalle ACTUAL" src="<?php echo($PATH_IMAGEN) ?>/Imagenes/seleccionar.gif" alt="Seleccionar" border="0" height="15" width="15">
							</td>
						</tr>
					</table>
					</a>
				...
				<td class="elementotabla columna_tabla_sel col_id" >
					<table>
						<tr>
							<td>
								<input id="t-id_<?php echo($i) ?>" name="t-id_<?php echo($i) ?>" type="checkbox" class="chkb_id" title="SELECCIONAR Estimado Detalle ACTUAL" value="<?php echo($estimado_detalle->id) ?>"></input>
							</td>
							<td>
								<input id="t-cel_<?php echo($i) ?>_0" name="t-cel_<?php echo($i) ?>_0" type="text" maxlength="25" value="<?php echo($estimado_detalle->id) ?>" style="width:50px" readonly></input>
							</td>
						</tr>
					</table>
				</td>
					...
					<td class="elementotabla col_created_at"  style="display:none"> 
						<?php echo( $estimado_detalle->updated_at ) ?>
					</td>
				
					<td class="elementotabla col_updated_at"  style="display:none"> 
						<?php echo( $estimado_detalle->updated_at ) ?>
					</td>
				<td class="elementotabla col_id_estimado" > <?php echo($estimado_detalle->id_estimado_Descripcion) ?></td>
				<td class="elementotabla col_id_bodega" > <?php echo($estimado_detalle->id_bodega_Descripcion) ?></td>
				<td class="elementotabla col_id_producto" > <?php echo($estimado_detalle->id_producto_Descripcion) ?></td>
				<td class="elementotabla col_id_unidad" > <?php echo($estimado_detalle->id_unidad_Descripcion) ?></td>
				
					<td class="elementotabla col_cantidad" > 
						<?php echo( $estimado_detalle->cantidad ) ?>
					</td>
				
					<td class="elementotabla col_precio" > 
						<?php echo( $estimado_detalle->precio ) ?>
					</td>
				
					<td class="elementotabla col_descuento_porciento" > 
						<?php echo( $estimado_detalle->descuento_porciento ) ?>
					</td>
				
					<td class="elementotabla col_descuento_monto" > 
						<?php echo( $estimado_detalle->descuento_monto ) ?>
					</td>
				
					<td class="elementotabla col_sub_total" > 
						<?php echo( $estimado_detalle->sub_total ) ?>
					</td>

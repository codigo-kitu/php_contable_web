...
*/
namespace com\bydan\contabilidad\cuentascobrar\debito_cuenta_cobrar\business\entity;
...
class debito_cuenta_cobrar extends GeneralEntity {

	/*GENERAL*/
	public static string $class='debito_cuenta_cobrar';
	
	/*AUXILIARES*/
	public ?debito_cuenta_cobrar $debito_cuenta_cobrar_Original=null;	
	public ?GeneralEntity $debito_cuenta_cobrar_Additional=null;
	public array $map_debito_cuenta_cobrar=array();	
	...
	function __construct () {
		...
		$this->debito_cuenta_cobrar_Original=$this;
		*/
		...
		$this->debito_cuenta_cobrar_Additional=new debito_cuenta_cobrar_Additional();*/
    } 
	...
	public function inicializarMap_debito_cuenta_cobrar() {
		$this->map_debito_cuenta_cobrar = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->id_cuenta_cobrar!==$newid_cuenta_cobrar) {
				if($newid_cuenta_cobrar===null && $newid_cuenta_cobrar!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_cuenta_cobrar');
				}

				if($newid_cuenta_cobrar!==null && filter_var($newid_cuenta_cobrar,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_cuenta_cobrar='.$newid_cuenta_cobrar);
				}
		...
		try {
			if($this->numero!==$newnumero) {
				if($newnumero===null && $newnumero!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna numero');
				}

				if(strlen($newnumero)>12) {
					throw new Exception('debito_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=12 en columna numero');
				}

				if(filter_var($newnumero,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('debito_cuenta_cobrar:Formato incorrecto en columna numero='.$newnumero);
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('debito_cuenta_cobrar:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->fecha_vence!==$newfecha_vence) {
				if($newfecha_vence===null && $newfecha_vence!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna fecha_vence');
				}
				...
				if($newfecha_vence!==null && filter_var($newfecha_vence,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('debito_cuenta_cobrar:No es fecha - fecha_vence='.$newfecha_vence);
				}
		...
		try {
			if($this->id_termino_pago_cliente!==$newid_termino_pago_cliente) {
				if($newid_termino_pago_cliente===null && $newid_termino_pago_cliente!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_termino_pago_cliente');
				}

				if($newid_termino_pago_cliente!==null && filter_var($newid_termino_pago_cliente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_termino_pago_cliente='.$newid_termino_pago_cliente);
				}
		...
		try {
			if($this->monto!==$newmonto) {
				if($newmonto===null && $newmonto!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna monto');
				}

				if($newmonto!=0) {
					if($newmonto!==null && filter_var($newmonto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('debito_cuenta_cobrar:No es numero decimal - monto='.$newmonto);
					}
				}
		...
		try {
			if($this->saldo!==$newsaldo) {
				if($newsaldo===null && $newsaldo!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna saldo');
				}

				if($newsaldo!=0) {
					if($newsaldo!==null && filter_var($newsaldo,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('debito_cuenta_cobrar:No es numero decimal - saldo='.$newsaldo);
					}
				}
	...
	public function setdescripcion(?string $newdescripcion) {
		if($this->descripcion!==$newdescripcion) {
				...
				if(strlen($newdescripcion)>1000) {
					try {
						throw new Exception('debito_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=descripcion en columna descripcion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newdescripcion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('debito_cuenta_cobrar:Formato incorrecto en la columna descripcion='.$newdescripcion);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->sub_total!==$newsub_total) {
				if($newsub_total===null && $newsub_total!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna sub_total');
				}

				if($newsub_total!=0) {
					if($newsub_total!==null && filter_var($newsub_total,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('debito_cuenta_cobrar:No es numero decimal - sub_total='.$newsub_total);
					}
				}
		...
		try {
			if($this->iva!==$newiva) {
				if($newiva===null && $newiva!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna iva');
				}

				if($newiva!=0) {
					if($newiva!==null && filter_var($newiva,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('debito_cuenta_cobrar:No es numero decimal - iva='.$newiva);
					}
				}
		...
		try {
			if($this->es_balance_inicial!==$newes_balance_inicial) {
				if($newes_balance_inicial===null && $newes_balance_inicial!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna es_balance_inicial');
				}
				...
				if($newes_balance_inicial!==null && filter_var($newes_balance_inicial,FILTER_VALIDATE_BOOLEAN)===false && $newes_balance_inicial!==0 && $newes_balance_inicial!==false) {
					throw new Exception('debito_cuenta_cobrar:No es valor booleano - es_balance_inicial='.$newes_balance_inicial);
				}
		...
		try {
			if($this->id_estado!==$newid_estado) {
				if($newid_estado===null && $newid_estado!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna id_estado');
				}

				if($newid_estado!==null && filter_var($newid_estado,FILTER_VALIDATE_INT)===false) {
					throw new Exception('debito_cuenta_cobrar:No es numero entero - id_estado='.$newid_estado);
				}
	...
	public function setreferencia(?string $newreferencia) {
		if($this->referencia!==$newreferencia) {
				...
				if(strlen($newreferencia)>25) {
					try {
						throw new Exception('debito_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=referencia en columna referencia');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newreferencia,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('debito_cuenta_cobrar:Formato incorrecto en la columna referencia='.$newreferencia);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->debito!==$newdebito) {
				if($newdebito===null && $newdebito!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna debito');
				}

				if($newdebito!=0) {
					if($newdebito!==null && filter_var($newdebito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('debito_cuenta_cobrar:No es numero decimal - debito='.$newdebito);
					}
				}
		...
		try {
			if($this->credito!==$newcredito) {
				if($newcredito===null && $newcredito!='') {
					throw new Exception('debito_cuenta_cobrar:Valor nulo no permitido en columna credito');
				}

				if($newcredito!=0) {
					if($newcredito!==null && filter_var($newcredito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('debito_cuenta_cobrar:No es numero decimal - credito='.$newcredito);
					}
				}
	...
	/*AUXILIARES*/
	public function setMap_debito_cuenta_cobrarValue(string $sKey,$oValue) {
		$this->map_debito_cuenta_cobrar[$sKey]=$oValue;
	}
	
	public function getMap_debito_cuenta_cobrarValue(string $sKey) {
		return $this->map_debito_cuenta_cobrar[$sKey];
	}
	
	public function getdebito_cuenta_cobrar_Original() : ?debito_cuenta_cobrar {
		return $this->debito_cuenta_cobrar_Original;
	}
	
	public function setdebito_cuenta_cobrar_Original(debito_cuenta_cobrar $debito_cuenta_cobrar) {
		try {
			$this->debito_cuenta_cobrar_Original=$debito_cuenta_cobrar;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_debito_cuenta_cobrar() : array {
		return $this->map_debito_cuenta_cobrar;
	}

	public function setMap_debito_cuenta_cobrar(array $map_debito_cuenta_cobrar) {
		$this->map_debito_cuenta_cobrar = $map_debito_cuenta_cobrar;
	}

			...
			<?php foreach ($categoria_clientesLocal as $categoria_cliente) { ?>
				...
				<td class="elementotabla col_id" >
					...
					<table>
						<tr>
							...
							<td>
								<img class="imgseleccionarcategoria_cliente" idactualcategoria_cliente="<?php echo($categoria_cliente->id) ?>"  funcionjsactualcategoria_cliente="'.str_replace('TO_REPLACE',$categoria_cliente->id.',\''.categoria_cliente_util::getcategoria_clienteDescripcion($categoria_cliente).'\'',$this->strFuncionJs).'" title="SELECCIONAR Categorias Cliente ACTUAL" src="<?php echo($PATH_IMAGEN) ?>/Imagenes/seleccionar.gif" alt="Seleccionar" border="0" height="15" width="15">
							</td>
						</tr>
					</table>
					</a>
				...
				<td class="elementotabla columna_tabla_sel col_id" >
					<table>
						<tr>
							<td>
								<input id="t-id_<?php echo($i) ?>" name="t-id_<?php echo($i) ?>" type="checkbox" class="chkb_id" title="SELECCIONAR Categorias Cliente ACTUAL" value="<?php echo($categoria_cliente->id) ?>"></input>
							</td>
							<td>
								<input id="t-cel_<?php echo($i) ?>_0" name="t-cel_<?php echo($i) ?>_0" type="text" maxlength="25" value="<?php echo($categoria_cliente->id) ?>" style="width:50px" readonly></input>
							</td>
						</tr>
					</table>
				</td>
					...
					<td class="elementotabla col_created_at"  style="display:none"> 
						<?php echo( $categoria_cliente->updated_at ) ?>
					</td>
					...
					<td class="elementotabla col_nombre" > 
						<?php echo( $categoria_cliente->nombre ) ?>
					</td>
				
					<td class="elementotabla col_predeterminado" ><?php  $funciones->getCheckBox($categoria_cliente->predeterminado,'form<?php echo($strSufijo) ?>-predeterminadoi',$paraReporte)  ?>
					</td>

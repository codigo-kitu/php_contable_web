...
*/
namespace com\bydan\contabilidad\cuentascobrar\forma_pago_cliente\business\entity;
...
class forma_pago_cliente extends GeneralEntity {

	/*GENERAL*/
	public static string $class='forma_pago_cliente';
	
	/*AUXILIARES*/
	public ?forma_pago_cliente $forma_pago_cliente_Original=null;	
	public ?GeneralEntity $forma_pago_cliente_Additional=null;
	public array $map_forma_pago_cliente=array();	
	...
	public array $documentocuentacobrars = array();
	...
	function __construct () {
		...
		$this->forma_pago_cliente_Original=$this;
		*/
		...
		$this->documentocuentacobrars=array();
		$this->pagocuentacobrars=array();
		$this->creditocuentacobrars=array();
		
		/*AUTOREFERENCIA INFINITA TALVEZ
		$this->forma_pago_cliente_Additional=new forma_pago_cliente_Additional();*/
    } 
	...
	public function inicializarMap_forma_pago_cliente() {
		$this->map_forma_pago_cliente = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('forma_pago_cliente:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('forma_pago_cliente:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_tipo_forma_pago!==$newid_tipo_forma_pago) {
				if($newid_tipo_forma_pago===null && $newid_tipo_forma_pago!='') {
					throw new Exception('forma_pago_cliente:Valor nulo no permitido en columna id_tipo_forma_pago');
				}

				if($newid_tipo_forma_pago!==null && filter_var($newid_tipo_forma_pago,FILTER_VALIDATE_INT)===false) {
					throw new Exception('forma_pago_cliente:No es numero entero - id_tipo_forma_pago='.$newid_tipo_forma_pago);
				}
		...
		try {
			if($this->codigo!==$newcodigo) {
				if($newcodigo===null && $newcodigo!='') {
					throw new Exception('forma_pago_cliente:Valor nulo no permitido en columna codigo');
				}

				if(strlen($newcodigo)>4) {
					throw new Exception('forma_pago_cliente:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=4 en columna codigo');
				}

				if(filter_var($newcodigo,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('forma_pago_cliente:Formato incorrecto en columna codigo='.$newcodigo);
				}
		...
		try {
			if($this->nombre!==$newnombre) {
				if($newnombre===null && $newnombre!='') {
					throw new Exception('forma_pago_cliente:Valor nulo no permitido en columna nombre');
				}

				if(strlen($newnombre)>50) {
					throw new Exception('forma_pago_cliente:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=50 en columna nombre');
				}

				if(filter_var($newnombre,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('forma_pago_cliente:Formato incorrecto en columna nombre='.$newnombre);
				}
		...
		try {
			if($this->predeterminado!==$newpredeterminado) {
				if($newpredeterminado===null && $newpredeterminado!='') {
					throw new Exception('forma_pago_cliente:Valor nulo no permitido en columna predeterminado');
				}
				...
				if($newpredeterminado!==null && filter_var($newpredeterminado,FILTER_VALIDATE_BOOLEAN)===false && $newpredeterminado!==0 && $newpredeterminado!==false) {
					throw new Exception('forma_pago_cliente:No es valor booleano - predeterminado='.$newpredeterminado);
				}
	...
	public function setid_cuenta(?int $newid_cuenta) {
		if($this->id_cuenta!==$newid_cuenta) {
				...
				if($newid_cuenta!==null && filter_var($newid_cuenta,FILTER_VALIDATE_INT)===false) {
					throw new Exception('forma_pago_cliente:No es numero entero - id_cuenta');
				}
	...
	public function setcuenta_contable(?string $newcuenta_contable) {
		if($this->cuenta_contable!==$newcuenta_contable) {
				...
				if(strlen($newcuenta_contable)>20) {
					try {
						throw new Exception('forma_pago_cliente:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=cuenta_contable en columna cuenta_contable');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newcuenta_contable,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('forma_pago_cliente:Formato incorrecto en la columna cuenta_contable='.$newcuenta_contable);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function getdocumento_cuenta_cobrars() : array {
		return $this->documentocuentacobrars;
	}
	...
	public  function  setdocumento_cuenta_cobrars(array $documentocuentacobrars) {
		try {
			$this->documentocuentacobrars=$documentocuentacobrars;
		} catch(Exception $e) {
			;
		}
	}
	...
	/*AUXILIARES*/
	public function setMap_forma_pago_clienteValue(string $sKey,$oValue) {
		$this->map_forma_pago_cliente[$sKey]=$oValue;
	}
	
	public function getMap_forma_pago_clienteValue(string $sKey) {
		return $this->map_forma_pago_cliente[$sKey];
	}
	
	public function getforma_pago_cliente_Original() : ?forma_pago_cliente {
		return $this->forma_pago_cliente_Original;
	}
	
	public function setforma_pago_cliente_Original(forma_pago_cliente $forma_pago_cliente) {
		try {
			$this->forma_pago_cliente_Original=$forma_pago_cliente;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_forma_pago_cliente() : array {
		return $this->map_forma_pago_cliente;
	}

	public function setMap_forma_pago_cliente(array $map_forma_pago_cliente) {
		$this->map_forma_pago_cliente = $map_forma_pago_cliente;
	}

...
*/
namespace com\bydan\contabilidad\cuentascobrar\documento_cuenta_cobrar\business\entity;
...
class documento_cuenta_cobrar extends GeneralEntity {

	/*GENERAL*/
	public static string $class='documento_cuenta_cobrar';
	
	/*AUXILIARES*/
	public ?documento_cuenta_cobrar $documento_cuenta_cobrar_Original=null;	
	public ?GeneralEntity $documento_cuenta_cobrar_Additional=null;
	public array $map_documento_cuenta_cobrar=array();	
	...
	public array $imagendocumentocuentacobrars = array();
	...
	function __construct () {
		...
		$this->documento_cuenta_cobrar_Original=$this;
		*/
		...
		$this->facturas=array();
		$this->imagendocumentocuentacobrars=array();
		$this->facturalotes=array();
		$this->devolucionfacturas=array();
		
		/*AUTOREFERENCIA INFINITA TALVEZ
		$this->documento_cuenta_cobrar_Additional=new documento_cuenta_cobrar_Additional();*/
    } 
	...
	public function inicializarMap_documento_cuenta_cobrar() {
		$this->map_documento_cuenta_cobrar = array();
	}
	...
	public function  getimpuesto_incluido() : ?string {
	...
	public function  getref_pago() : ?string {
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->numero!==$newnumero) {
				if($newnumero===null && $newnumero!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna numero');
				}

				if(strlen($newnumero)>250) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=250 en columna numero');
				}

				if(filter_var($newnumero,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna numero='.$newnumero);
				}
		...
		try {
			if($this->id_cliente!==$newid_cliente) {
				if($newid_cliente===null && $newid_cliente!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_cliente');
				}

				if($newid_cliente!==null && filter_var($newid_cliente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_cliente='.$newid_cliente);
				}
		...
		try {
			if($this->tipo!==$newtipo) {
				if($newtipo===null && $newtipo!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna tipo');
				}

				if(strlen($newtipo)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna tipo');
				}

				if(filter_var($newtipo,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna tipo='.$newtipo);
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('documento_cuenta_cobrar:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->descripcion!==$newdescripcion) {
				if($newdescripcion===null && $newdescripcion!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna descripcion');
				}

				if(strlen($newdescripcion)>120) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=120 en columna descripcion');
				}
				...
				if(filter_var($newdescripcion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna descripcion='.$newdescripcion);
				}
		...
		try {
			if($this->monto!==$newmonto) {
				if($newmonto===null && $newmonto!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna monto');
				}

				if($newmonto!=0) {
					if($newmonto!==null && filter_var($newmonto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - monto='.$newmonto);
					}
				}
		...
		try {
			if($this->monto_parcial!==$newmonto_parcial) {
				if($newmonto_parcial===null && $newmonto_parcial!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna monto_parcial');
				}

				if($newmonto_parcial!=0) {
					if($newmonto_parcial!==null && filter_var($newmonto_parcial,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - monto_parcial='.$newmonto_parcial);
					}
				}
		...
		try {
			if($this->moneda!==$newmoneda) {
				if($newmoneda===null && $newmoneda!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna moneda');
				}

				if(strlen($newmoneda)>1) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=1 en columna moneda');
				}

				if(filter_var($newmoneda,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna moneda='.$newmoneda);
				}
		...
		try {
			if($this->fecha_vence!==$newfecha_vence) {
				if($newfecha_vence===null && $newfecha_vence!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna fecha_vence');
				}
				...
				if($newfecha_vence!==null && filter_var($newfecha_vence,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('documento_cuenta_cobrar:No es fecha - fecha_vence='.$newfecha_vence);
				}
		...
		try {
			if($this->numero_de_pagos!==$newnumero_de_pagos) {
				if($newnumero_de_pagos===null && $newnumero_de_pagos!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna numero_de_pagos');
				}

				if($newnumero_de_pagos!==null && filter_var($newnumero_de_pagos,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - numero_de_pagos='.$newnumero_de_pagos);
				}
		...
		try {
			if($this->balance!==$newbalance) {
				if($newbalance===null && $newbalance!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna balance');
				}

				if($newbalance!=0) {
					if($newbalance!==null && filter_var($newbalance,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - balance='.$newbalance);
					}
				}
		...
		try {
			if($this->numero_pagado!==$newnumero_pagado) {
				if($newnumero_pagado===null && $newnumero_pagado!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna numero_pagado');
				}

				if(strlen($newnumero_pagado)>10) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=10 en columna numero_pagado');
				}
				...
				if(filter_var($newnumero_pagado,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna numero_pagado='.$newnumero_pagado);
				}
		...
		try {
			if($this->id_pagado!==$newid_pagado) {
				if($newid_pagado===null && $newid_pagado!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_pagado');
				}

				if($newid_pagado!==null && filter_var($newid_pagado,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_pagado='.$newid_pagado);
				}
		...
		try {
			if($this->modulo_origen!==$newmodulo_origen) {
				if($newmodulo_origen===null && $newmodulo_origen!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna modulo_origen');
				}

				if(strlen($newmodulo_origen)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna modulo_origen');
				}
				...
				if(filter_var($newmodulo_origen,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna modulo_origen='.$newmodulo_origen);
				}
		...
		try {
			if($this->id_origen!==$newid_origen) {
				if($newid_origen===null && $newid_origen!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_origen');
				}

				if($newid_origen!==null && filter_var($newid_origen,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_origen='.$newid_origen);
				}
		...
		try {
			if($this->modulo_destino!==$newmodulo_destino) {
				if($newmodulo_destino===null && $newmodulo_destino!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna modulo_destino');
				}

				if(strlen($newmodulo_destino)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna modulo_destino');
				}
				...
				if(filter_var($newmodulo_destino,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna modulo_destino='.$newmodulo_destino);
				}
		...
		try {
			if($this->id_destino!==$newid_destino) {
				if($newid_destino===null && $newid_destino!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_destino');
				}

				if($newid_destino!==null && filter_var($newid_destino,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_destino='.$newid_destino);
				}
		...
		try {
			if($this->nombre_pc!==$newnombre_pc) {
				if($newnombre_pc===null && $newnombre_pc!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna nombre_pc');
				}

				if(strlen($newnombre_pc)>100) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=100 en columna nombre_pc');
				}

				if(filter_var($newnombre_pc,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna nombre_pc='.$newnombre_pc);
				}
		...
		try {
			if($this->hora!==$newhora) {
				if($newhora===null && $newhora!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna hora');
				}

				if($newhora!==null && filter_var($newhora,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('documento_cuenta_cobrar:No es fecha - hora='.$newhora);
				}
		...
		try {
			if($this->monto_mora!==$newmonto_mora) {
				if($newmonto_mora===null && $newmonto_mora!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna monto_mora');
				}

				if($newmonto_mora!=0) {
					if($newmonto_mora!==null && filter_var($newmonto_mora,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - monto_mora='.$newmonto_mora);
					}
				}
		...
		try {
			if($this->interes_mora!==$newinteres_mora) {
				if($newinteres_mora===null && $newinteres_mora!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna interes_mora');
				}

				if($newinteres_mora!=0) {
					if($newinteres_mora!==null && filter_var($newinteres_mora,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - interes_mora='.$newinteres_mora);
					}
				}
		...
		try {
			if($this->dias_gracia_mora!==$newdias_gracia_mora) {
				if($newdias_gracia_mora===null && $newdias_gracia_mora!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna dias_gracia_mora');
				}

				if($newdias_gracia_mora!==null && filter_var($newdias_gracia_mora,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - dias_gracia_mora='.$newdias_gracia_mora);
				}
		...
		try {
			if($this->instrumento_pago!==$newinstrumento_pago) {
				if($newinstrumento_pago===null && $newinstrumento_pago!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna instrumento_pago');
				}

				if(strlen($newinstrumento_pago)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna instrumento_pago');
				}
				...
				if(filter_var($newinstrumento_pago,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna instrumento_pago='.$newinstrumento_pago);
				}
		...
		try {
			if($this->tipo_cambio!==$newtipo_cambio) {
				if($newtipo_cambio===null && $newtipo_cambio!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna tipo_cambio');
				}

				if($newtipo_cambio!=0) {
					if($newtipo_cambio!==null && filter_var($newtipo_cambio,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - tipo_cambio='.$newtipo_cambio);
					}
				}
	...
	public function setnumero_cliente(?string $newnumero_cliente)
	{
		try {
			if($this->numero_cliente!==$newnumero_cliente) {
				if($newnumero_cliente===null && $newnumero_cliente!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna numero_cliente');
				}

				if(strlen($newnumero_cliente)>30) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=30 en columna numero_cliente');
				}

				if(filter_var($newnumero_cliente,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna numero_cliente='.$newnumero_cliente);
				}
		...
		try {
			if($this->clase_registro!==$newclase_registro) {
				if($newclase_registro===null && $newclase_registro!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna clase_registro');
				}

				if(strlen($newclase_registro)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna clase_registro');
				}
				...
				if(filter_var($newclase_registro,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna clase_registro='.$newclase_registro);
				}
		...
		try {
			if($this->estado_registro!==$newestado_registro) {
				if($newestado_registro===null && $newestado_registro!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna estado_registro');
				}

				if(strlen($newestado_registro)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna estado_registro');
				}
				...
				if(filter_var($newestado_registro,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna estado_registro='.$newestado_registro);
				}
		...
		try {
			if($this->motivo_anulacion!==$newmotivo_anulacion) {
				if($newmotivo_anulacion===null && $newmotivo_anulacion!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna motivo_anulacion');
				}

				if(strlen($newmotivo_anulacion)>1000) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=1000 en columna motivo_anulacion');
				}
				...
				if(filter_var($newmotivo_anulacion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna motivo_anulacion='.$newmotivo_anulacion);
				}
		...
		try {
			if($this->impuesto_1!==$newimpuesto_1) {
				if($newimpuesto_1===null && $newimpuesto_1!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna impuesto_1');
				}

				if($newimpuesto_1!=0) {
					if($newimpuesto_1!==null && filter_var($newimpuesto_1,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - impuesto_1='.$newimpuesto_1);
					}
				}
		...
		try {
			if($this->impuesto_2!==$newimpuesto_2) {
				if($newimpuesto_2===null && $newimpuesto_2!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna impuesto_2');
				}

				if($newimpuesto_2!=0) {
					if($newimpuesto_2!==null && filter_var($newimpuesto_2,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('documento_cuenta_cobrar:No es numero decimal - impuesto_2='.$newimpuesto_2);
					}
				}
	...
	public function setimpuesto_incluido(?string $newimpuesto_incluido)
	{
		try {
			if($this->impuesto_incluido!==$newimpuesto_incluido) {
				if($newimpuesto_incluido===null && $newimpuesto_incluido!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna impuesto_incluido');
				}

				if(strlen($newimpuesto_incluido)>2) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=2 en columna impuesto_incluido');
				}

				if(filter_var($newimpuesto_incluido,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna impuesto_incluido='.$newimpuesto_incluido);
				}
		...
		try {
			if($this->observaciones!==$newobservaciones) {
				if($newobservaciones===null && $newobservaciones!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna observaciones');
				}

				if(strlen($newobservaciones)>1000) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=1000 en columna observaciones');
				}
				...
				if(filter_var($newobservaciones,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna observaciones='.$newobservaciones);
				}
		...
		try {
			if($this->grupo_pago!==$newgrupo_pago) {
				if($newgrupo_pago===null && $newgrupo_pago!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna grupo_pago');
				}

				if(strlen($newgrupo_pago)>100) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=100 en columna grupo_pago');
				}
				...
				if(filter_var($newgrupo_pago,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna grupo_pago='.$newgrupo_pago);
				}
		...
		try {
			if($this->termino_idpv!==$newtermino_idpv) {
				if($newtermino_idpv===null && $newtermino_idpv!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna termino_idpv');
				}

				if($newtermino_idpv!==null && filter_var($newtermino_idpv,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - termino_idpv='.$newtermino_idpv);
				}
		...
		try {
			if($this->id_forma_pago_cliente!==$newid_forma_pago_cliente) {
				if($newid_forma_pago_cliente===null && $newid_forma_pago_cliente!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_forma_pago_cliente');
				}

				if($newid_forma_pago_cliente!==null && filter_var($newid_forma_pago_cliente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_forma_pago_cliente='.$newid_forma_pago_cliente);
				}
		...
		try {
			if($this->nro_pago!==$newnro_pago) {
				if($newnro_pago===null && $newnro_pago!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna nro_pago');
				}

				if(strlen($newnro_pago)>30) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=30 en columna nro_pago');
				}

				if(filter_var($newnro_pago,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna nro_pago='.$newnro_pago);
				}
	...
	public function setref_pago(?string $newref_pago)
	{
		try {
			if($this->ref_pago!==$newref_pago) {
				if($newref_pago===null && $newref_pago!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna ref_pago');
				}

				if(strlen($newref_pago)>80) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=80 en columna ref_pago');
				}

				if(filter_var($newref_pago,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna ref_pago='.$newref_pago);
				}

				$this->ref_pago=$newref_pago;
				$this->setIsChanged(true);
			}
		} catch(Exception $e) {
			throw $e;
		...
		try {
			if($this->fecha_hora!==$newfecha_hora) {
				if($newfecha_hora===null && $newfecha_hora!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna fecha_hora');
				}
				...
				if($newfecha_hora!==null && filter_var($newfecha_hora,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('documento_cuenta_cobrar:No es fecha - fecha_hora='.$newfecha_hora);
				}
		...
		try {
			if($this->id_base!==$newid_base) {
				if($newid_base===null && $newid_base!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_base');
				}

				if($newid_base!==null && filter_var($newid_base,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_base='.$newid_base);
				}
		...
		try {
			if($this->id_cuenta_corriente!==$newid_cuenta_corriente) {
				if($newid_cuenta_corriente===null && $newid_cuenta_corriente!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna id_cuenta_corriente');
				}

				if($newid_cuenta_corriente!==null && filter_var($newid_cuenta_corriente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('documento_cuenta_cobrar:No es numero entero - id_cuenta_corriente='.$newid_cuenta_corriente);
				}
	...
	public function setncf(?string $newncf)
	{
		try {
			if($this->ncf!==$newncf) {
				if($newncf===null && $newncf!='') {
					throw new Exception('documento_cuenta_cobrar:Valor nulo no permitido en columna ncf');
				}

				if(strlen($newncf)>50) {
					throw new Exception('documento_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=50 en columna ncf');
				}

				if(filter_var($newncf,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('documento_cuenta_cobrar:Formato incorrecto en columna ncf='.$newncf);
				}
	...
	public function getimagen_documento_cuenta_cobrars() : array {
		return $this->imagendocumentocuentacobrars;
	}
	...
	public  function  setimagen_documento_cuenta_cobrars(array $imagendocumentocuentacobrars) {
		try {
			$this->imagendocumentocuentacobrars=$imagendocumentocuentacobrars;
		} catch(Exception $e) {
			;
		}
	}
	...
	/*AUXILIARES*/
	public function setMap_documento_cuenta_cobrarValue(string $sKey,$oValue) {
		$this->map_documento_cuenta_cobrar[$sKey]=$oValue;
	}
	
	public function getMap_documento_cuenta_cobrarValue(string $sKey) {
		return $this->map_documento_cuenta_cobrar[$sKey];
	}
	
	public function getdocumento_cuenta_cobrar_Original() : ?documento_cuenta_cobrar {
		return $this->documento_cuenta_cobrar_Original;
	}
	
	public function setdocumento_cuenta_cobrar_Original(documento_cuenta_cobrar $documento_cuenta_cobrar) {
		try {
			$this->documento_cuenta_cobrar_Original=$documento_cuenta_cobrar;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_documento_cuenta_cobrar() : array {
		return $this->map_documento_cuenta_cobrar;
	}

	public function setMap_documento_cuenta_cobrar(array $map_documento_cuenta_cobrar) {
		$this->map_documento_cuenta_cobrar = $map_documento_cuenta_cobrar;
	}

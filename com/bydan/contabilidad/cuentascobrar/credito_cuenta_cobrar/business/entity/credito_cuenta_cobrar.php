...
*/
namespace com\bydan\contabilidad\cuentascobrar\credito_cuenta_cobrar\business\entity;
...
class credito_cuenta_cobrar extends GeneralEntity {

	/*GENERAL*/
	public static string $class='credito_cuenta_cobrar';
	
	/*AUXILIARES*/
	public ?credito_cuenta_cobrar $credito_cuenta_cobrar_Original=null;	
	public ?GeneralEntity $credito_cuenta_cobrar_Additional=null;
	public array $map_credito_cuenta_cobrar=array();	
	...
	function __construct () {
		...
		$this->credito_cuenta_cobrar_Original=$this;
		*/
		...
		$this->credito_cuenta_cobrar_Additional=new credito_cuenta_cobrar_Additional();*/
    } 
	...
	public function inicializarMap_credito_cuenta_cobrar() {
		$this->map_credito_cuenta_cobrar = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->id_cuenta_cobrar!==$newid_cuenta_cobrar) {
				if($newid_cuenta_cobrar===null && $newid_cuenta_cobrar!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_cuenta_cobrar');
				}

				if($newid_cuenta_cobrar!==null && filter_var($newid_cuenta_cobrar,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_cuenta_cobrar='.$newid_cuenta_cobrar);
				}
		...
		try {
			if($this->numero!==$newnumero) {
				if($newnumero===null && $newnumero!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna numero');
				}

				if(strlen($newnumero)>12) {
					throw new Exception('credito_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=12 en columna numero');
				}

				if(filter_var($newnumero,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					throw new Exception('credito_cuenta_cobrar:Formato incorrecto en columna numero='.$newnumero);
				}
		...
		try {
			if($this->id_forma_pago_cliente!==$newid_forma_pago_cliente) {
				if($newid_forma_pago_cliente===null && $newid_forma_pago_cliente!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna id_forma_pago_cliente');
				}

				if($newid_forma_pago_cliente!==null && filter_var($newid_forma_pago_cliente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_forma_pago_cliente='.$newid_forma_pago_cliente);
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('credito_cuenta_cobrar:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->fecha_vence!==$newfecha_vence) {
				if($newfecha_vence===null && $newfecha_vence!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna fecha_vence');
				}
				...
				if($newfecha_vence!==null && filter_var($newfecha_vence,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('credito_cuenta_cobrar:No es fecha - fecha_vence='.$newfecha_vence);
				}
		...
		try {
			if($this->abono!==$newabono) {
				if($newabono===null && $newabono!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna abono');
				}

				if($newabono!=0) {
					if($newabono!==null && filter_var($newabono,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('credito_cuenta_cobrar:No es numero decimal - abono='.$newabono);
					}
				}
		...
		try {
			if($this->saldo!==$newsaldo) {
				if($newsaldo===null && $newsaldo!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna saldo');
				}

				if($newsaldo!=0) {
					if($newsaldo!==null && filter_var($newsaldo,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('credito_cuenta_cobrar:No es numero decimal - saldo='.$newsaldo);
					}
				}
	...
	public function setdescripcion(?string $newdescripcion) {
		if($this->descripcion!==$newdescripcion) {
				...
				if(strlen($newdescripcion)>1000) {
					try {
						throw new Exception('credito_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=descripcion en columna descripcion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newdescripcion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('credito_cuenta_cobrar:Formato incorrecto en la columna descripcion='.$newdescripcion);
					} catch(Exception $e) {
						throw $e;
					}
				}
	...
	public function setid_estado(?int $newid_estado) {
		if($this->id_estado!==$newid_estado) {
				...
				if($newid_estado!==null && filter_var($newid_estado,FILTER_VALIDATE_INT)===false) {
					throw new Exception('credito_cuenta_cobrar:No es numero entero - id_estado');
				}
	...
	public function setreferencia(?string $newreferencia) {
		if($this->referencia!==$newreferencia) {
				...
				if(strlen($newreferencia)>25) {
					try {
						throw new Exception('credito_cuenta_cobrar:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=referencia en columna referencia');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newreferencia,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('credito_cuenta_cobrar:Formato incorrecto en la columna referencia='.$newreferencia);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->debito!==$newdebito) {
				if($newdebito===null && $newdebito!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna debito');
				}

				if($newdebito!=0) {
					if($newdebito!==null && filter_var($newdebito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('credito_cuenta_cobrar:No es numero decimal - debito='.$newdebito);
					}
				}
		...
		try {
			if($this->credito!==$newcredito) {
				if($newcredito===null && $newcredito!='') {
					throw new Exception('credito_cuenta_cobrar:Valor nulo no permitido en columna credito');
				}

				if($newcredito!=0) {
					if($newcredito!==null && filter_var($newcredito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('credito_cuenta_cobrar:No es numero decimal - credito='.$newcredito);
					}
				}
	...
	/*AUXILIARES*/
	public function setMap_credito_cuenta_cobrarValue(string $sKey,$oValue) {
		$this->map_credito_cuenta_cobrar[$sKey]=$oValue;
	}
	
	public function getMap_credito_cuenta_cobrarValue(string $sKey) {
		return $this->map_credito_cuenta_cobrar[$sKey];
	}
	
	public function getcredito_cuenta_cobrar_Original() : ?credito_cuenta_cobrar {
		return $this->credito_cuenta_cobrar_Original;
	}
	
	public function setcredito_cuenta_cobrar_Original(credito_cuenta_cobrar $credito_cuenta_cobrar) {
		try {
			$this->credito_cuenta_cobrar_Original=$credito_cuenta_cobrar;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_credito_cuenta_cobrar() : array {
		return $this->map_credito_cuenta_cobrar;
	}

	public function setMap_credito_cuenta_cobrar(array $map_credito_cuenta_cobrar) {
		$this->map_credito_cuenta_cobrar = $map_credito_cuenta_cobrar;
	}

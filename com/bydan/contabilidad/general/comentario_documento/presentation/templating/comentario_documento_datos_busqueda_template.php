			...
			<?php foreach ($comentario_documentosLocal as $comentario_documento) { ?>
				...
				<td class="elementotabla col_id" >
					...
					<table>
						<tr>
							...
							<td>
								<img class="imgseleccionarcomentario_documento" idactualcomentario_documento="<?php echo($comentario_documento->id) ?>"  funcionjsactualcomentario_documento="'.str_replace('TO_REPLACE',$comentario_documento->id.',\''.comentario_documento_util::getcomentario_documentoDescripcion($comentario_documento).'\'',$this->strFuncionJs).'" title="SELECCIONAR Comentario Documento ACTUAL" src="<?php echo($PATH_IMAGEN) ?>/Imagenes/seleccionar.gif" alt="Seleccionar" border="0" height="15" width="15">
							</td>
						</tr>
					</table>
					</a>
				...
				<td class="elementotabla columna_tabla_sel col_id" >
					<table>
						<tr>
							<td>
								<input id="t-id_<?php echo($i) ?>" name="t-id_<?php echo($i) ?>" type="checkbox" class="chkb_id" title="SELECCIONAR Comentario Documento ACTUAL" value="<?php echo($comentario_documento->id) ?>"></input>
							</td>
							<td>
								<input id="t-cel_<?php echo($i) ?>_0" name="t-cel_<?php echo($i) ?>_0" type="text" maxlength="25" value="<?php echo($comentario_documento->id) ?>" style="width:50px" readonly></input>
							</td>
						</tr>
					</table>
				</td>
					...
					<td class="elementotabla col_created_at"  style="display:none"> 
						<?php echo( $comentario_documento->updated_at ) ?>
					</td>
					...
					<td class="elementotabla col_tipo_documento" > 
						<?php echo( $comentario_documento->tipo_documento ) ?>
					</td>
				
					<td class="elementotabla col_comentario" > 
						<?php echo( $comentario_documento->comentario ) ?>
					</td>

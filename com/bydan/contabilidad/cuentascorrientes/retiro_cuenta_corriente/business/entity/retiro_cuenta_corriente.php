...
*/
namespace com\bydan\contabilidad\cuentascorrientes\retiro_cuenta_corriente\business\entity;
...
class retiro_cuenta_corriente extends GeneralEntity {

	/*GENERAL*/
	public static string $class='retiro_cuenta_corriente';
	
	/*AUXILIARES*/
	public ?retiro_cuenta_corriente $retiro_cuenta_corriente_Original=null;	
	public ?GeneralEntity $retiro_cuenta_corriente_Additional=null;
	public array $map_retiro_cuenta_corriente=array();	
	...
	function __construct () {
		...
		$this->retiro_cuenta_corriente_Original=$this;
		*/
		...
		$this->retiro_cuenta_corriente_Additional=new retiro_cuenta_corriente_Additional();*/
    } 
	...
	public function inicializarMap_retiro_cuenta_corriente() {
		$this->map_retiro_cuenta_corriente = array();
	}
		...
		try {
			if($this->id_empresa!==$newid_empresa) {
				if($newid_empresa===null && $newid_empresa!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_empresa');
				}

				if($newid_empresa!==null && filter_var($newid_empresa,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_empresa='.$newid_empresa);
				}
		...
		try {
			if($this->id_sucursal!==$newid_sucursal) {
				if($newid_sucursal===null && $newid_sucursal!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_sucursal');
				}

				if($newid_sucursal!==null && filter_var($newid_sucursal,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_sucursal='.$newid_sucursal);
				}
		...
		try {
			if($this->id_ejercicio!==$newid_ejercicio) {
				if($newid_ejercicio===null && $newid_ejercicio!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_ejercicio');
				}

				if($newid_ejercicio!==null && filter_var($newid_ejercicio,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_ejercicio='.$newid_ejercicio);
				}
		...
		try {
			if($this->id_periodo!==$newid_periodo) {
				if($newid_periodo===null && $newid_periodo!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_periodo');
				}

				if($newid_periodo!==null && filter_var($newid_periodo,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_periodo='.$newid_periodo);
				}
		...
		try {
			if($this->id_usuario!==$newid_usuario) {
				if($newid_usuario===null && $newid_usuario!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_usuario');
				}

				if($newid_usuario!==null && filter_var($newid_usuario,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_usuario='.$newid_usuario);
				}
		...
		try {
			if($this->id_cuenta_corriente!==$newid_cuenta_corriente) {
				if($newid_cuenta_corriente===null && $newid_cuenta_corriente!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_cuenta_corriente');
				}

				if($newid_cuenta_corriente!==null && filter_var($newid_cuenta_corriente,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_cuenta_corriente='.$newid_cuenta_corriente);
				}
		...
		try {
			if($this->fecha_emision!==$newfecha_emision) {
				if($newfecha_emision===null && $newfecha_emision!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna fecha_emision');
				}
				...
				if($newfecha_emision!==null && filter_var($newfecha_emision,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_FECHA)))===false) {
					throw new Exception('retiro_cuenta_corriente:No es fecha - fecha_emision='.$newfecha_emision);
				}
		...
		try {
			if($this->monto!==$newmonto) {
				if($newmonto===null && $newmonto!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna monto');
				}

				if($newmonto!=0) {
					if($newmonto!==null && filter_var($newmonto,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('retiro_cuenta_corriente:No es numero decimal - monto='.$newmonto);
					}
				}
	...
	public function setmonto_texto(?string $newmonto_texto) {
		if($this->monto_texto!==$newmonto_texto) {
				...
				if(strlen($newmonto_texto)>150) {
					try {
						throw new Exception('retiro_cuenta_corriente:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=monto_texto en columna monto_texto');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newmonto_texto,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('retiro_cuenta_corriente:Formato incorrecto en la columna monto_texto='.$newmonto_texto);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->saldo!==$newsaldo) {
				if($newsaldo===null && $newsaldo!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna saldo');
				}

				if($newsaldo!=0) {
					if($newsaldo!==null && filter_var($newsaldo,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('retiro_cuenta_corriente:No es numero decimal - saldo='.$newsaldo);
					}
				}
	...
	public function setdescripcion(?string $newdescripcion) {
		if($this->descripcion!==$newdescripcion) {
				...
				if(strlen($newdescripcion)>1000) {
					try {
						throw new Exception('retiro_cuenta_corriente:Ha sobrepasado el numero maximo de caracteres permitidos,Maximo=descripcion en columna descripcion');
					} catch(Exception $e) {
						throw $e;
					}
				}
				...
				if(filter_var($newdescripcion,FILTER_VALIDATE_REGEXP,array("options"=>array("regexp"=>Constantes::$S_REGEX_PHP_STRING_GENERAL)))===false) {
					try {
						throw new Exception('retiro_cuenta_corriente:Formato incorrecto en la columna descripcion='.$newdescripcion);
					} catch(Exception $e) {
						throw $e;
					}
				}
		...
		try {
			if($this->id_estado_deposito_retiro!==$newid_estado_deposito_retiro) {
				if($newid_estado_deposito_retiro===null && $newid_estado_deposito_retiro!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna id_estado_deposito_retiro');
				}

				if($newid_estado_deposito_retiro!==null && filter_var($newid_estado_deposito_retiro,FILTER_VALIDATE_INT)===false) {
					throw new Exception('retiro_cuenta_corriente:No es numero entero - id_estado_deposito_retiro='.$newid_estado_deposito_retiro);
				}
		...
		try {
			if($this->debito!==$newdebito) {
				if($newdebito===null && $newdebito!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna debito');
				}

				if($newdebito!=0) {
					if($newdebito!==null && filter_var($newdebito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('retiro_cuenta_corriente:No es numero decimal - debito='.$newdebito);
					}
				}
		...
		try {
			if($this->credito!==$newcredito) {
				if($newcredito===null && $newcredito!='') {
					throw new Exception('retiro_cuenta_corriente:Valor nulo no permitido en columna credito');
				}

				if($newcredito!=0) {
					if($newcredito!==null && filter_var($newcredito,FILTER_VALIDATE_FLOAT)===false) {
						throw new Exception('retiro_cuenta_corriente:No es numero decimal - credito='.$newcredito);
					}
				}
	...
	/*AUXILIARES*/
	public function setMap_retiro_cuenta_corrienteValue(string $sKey,$oValue) {
		$this->map_retiro_cuenta_corriente[$sKey]=$oValue;
	}
	
	public function getMap_retiro_cuenta_corrienteValue(string $sKey) {
		return $this->map_retiro_cuenta_corriente[$sKey];
	}
	
	public function getretiro_cuenta_corriente_Original() : ?retiro_cuenta_corriente {
		return $this->retiro_cuenta_corriente_Original;
	}
	
	public function setretiro_cuenta_corriente_Original(retiro_cuenta_corriente $retiro_cuenta_corriente) {
		try {
			$this->retiro_cuenta_corriente_Original=$retiro_cuenta_corriente;
		} catch(Exception $e) {
			;
		}
	}
	
	public function getMap_retiro_cuenta_corriente() : array {
		return $this->map_retiro_cuenta_corriente;
	}

	public function setMap_retiro_cuenta_corriente(array $map_retiro_cuenta_corriente) {
		$this->map_retiro_cuenta_corriente = $map_retiro_cuenta_corriente;
	}

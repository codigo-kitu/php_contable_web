...
class opcion_webcontrol extends opcion_webcontrol_add {
	...
	actualizarVariablesPagina(opcion_control) {
		...
		if(opcion_control.action=="index" || opcion_control.action=="load") {
		...
		} else if(opcion_control.action=="recargarFomularioGeneral") {
			...
			alert(opcion_control.action + " Revisar Manejo");
			
			if(opcion_control.bitEsAbrirVentanaAuxiliarUrl==true){
			...
			if(opcion_control.bitEsEjecutarFuncionJavaScript==true){
				this.actualizarPaginaEjecutarJavaScript(opcion_control);
			}
			...
			if(opcion_control.paramDivSeccion.bitDivsEsCargarGeneralAjaxWebPart==true) {		
				this.actualizarPaginaCargaGeneral(opcion_control);	
			}
			
			if(opcion_control.paramDivSeccion.bitDivEsCargarAjaxWebPart==true) {
				this.actualizarPaginaTablaDatos(opcion_control);
			}
			
			if(opcion_control.paramDivSeccion.bitDivsEsCargarMostrarAjaxWebPart==true) {		
			...
			if(opcion_control.paramDivSeccion.bitDivsEsCargarMostrarSoloBusquedasAjaxWebPart==true) {		
				this.actualizarPaginaAreaBusquedas(opcion_control);	
			}
			
			if(opcion_control.paramDivSeccion.bitDivsEsCargarMostrarSoloMantenimientosAjaxWebPart==true) {		
			...
			if(opcion_control.paramDivSeccion.bitDivEsCargarMantenimientosAjaxWebPart==true) {				
				this.actualizarPaginaTablaFilaActual(opcion_control);			
			}
			
			if(opcion_control.paramDivSeccion.bitDivsEsCargarReporteAuxiliarAjaxWebPart==true) {				
				this.actualizarPaginaImprimir(opcion_control);
			}
			
			
			if(opcion_control.paramDivSeccion.bitDivsEsCargarMostrarBusquedasAjaxWebPart==true) {
				this.actualizarPaginaAreaAuxiliarBusquedas(opcion_control);
			}
			
			if(opcion_control.paramDivSeccion.bitDivsEsCargarResumenAjaxWebPart==true) {						
				this.actualizarPaginaAreaAuxiliarResumen(opcion_control);
			}				
			
			if(opcion_control.paramDivSeccion.bitDivsEsCargarAccionesRelacionesAjaxWebPart==true) {						
				this.actualizarPaginaAreaAuxiliarAccionesRelaciones(opcion_control);
			}
			
			if(opcion_control.usuarioActual!=null && opcion_control.usuarioActual.field_strUserName!=null &&
			opcion_control.usuarioActual.field_strUserName!=undefined ) {
				
				this.actualizarPaginaUsuarioInvitado(opcion_control);			
			}
		}
...
//Funciones Auxiliares
	...
	actualizarPaginaTablaDatosAuxiliar(opcion_control) {
		if(opcion_funcion1.esPaginaForm()==true) {
	...
	actualizarPaginaEjecutarJavaScript(opcion_control) {
		eval(opcion_control.strFuncionJs);
	}
	
	actualizarPaginaMensajePantalla(opcion_control, mostrar) {
		
		if(mostrar==true) {
			opcion_funcion1.resaltarRestaurarDivsPagina(false);
			
			if(jQuery("#ParamsPostAccion-chbPostAccionSinCerrar").prop('checked')==false
				&& jQuery("#ParamsPostAccion-chbPostAccionNuevo").prop('checked')==false) {		
					
				opcion_funcion1.resaltarRestaurarDivMantenimiento(false);
			}			
	...
	actualizarPaginaImprimir(opcion_control) {
	
		funcionGeneral.printWebPartPage("opcion",opcion_control.htmlTablaReporteAuxiliars);			
			
		//jQuery("#divTablaDatosReporteAuxiliaropcionsAjaxWebPart").html(opcion_control.htmlTablaReporteAuxiliars);						
		//funcionGeneral.printWebPartPage("opcion",jQuery("#divTablaDatosReporteAuxiliaropcionsAjaxWebPart").html());			
	}
...
/*---------------------------------- AREA:FORMULARIO ---------------------------*/
	...
	actualizarCamposFormulario(opcion_control) {
		...
		jQuery("#form"+opcion_constante1.STR_SUFIJO+"-version_row").val(opcion_control.opcionActual.versionRow);
		...
		if(opcion_control.opcionActual.id_opcion!=null && opcion_control.opcionActual.id_opcion>-1){
			...
			//jQuery("#form"+opcion_constante1.STR_SUFIJO+"-id_opcion").select2("val", null);
	...
	actualizarSpanMensajesCampos(opcion_control) {
		...
		jQuery("#spanstrMensajeversion_row").text(opcion_control.strMensajeversion_row);		
...
/*---------------------------------- AREA:TABLA ---------------------------*/
	...
	registrarSesionParaperfil_opciones(idActual){
		...
		funcionGeneralEventoJQuery.registrarSesionMaestroParaDetalle(idActual,"opcion","perfil_opcion","seguridad","",opcion_funcion1,opcion_webcontrol1,"es","");
	}

	registrarSesionParaopciones(idActual){

		funcionGeneralEventoJQuery.registrarSesionMaestroParaDetalle(idActual,"opcion","opcion","seguridad","",opcion_funcion1,opcion_webcontrol1,"es","");
	}

	registrarSesionParaacciones(idActual){

		funcionGeneralEventoJQuery.registrarSesionMaestroParaDetalle(idActual,"opcion","accion","seguridad","",opcion_funcion1,opcion_webcontrol1,"es","");
	}

	registrarSesionParacampos(idActual){

		funcionGeneralEventoJQuery.registrarSesionMaestroParaDetalle(idActual,"opcion","campo","seguridad","",opcion_funcion1,opcion_webcontrol1,"s","");
	}
...
/*---------------------------------- AREA:AUXILIARES ---------------------------*/
	...
	registrarAccionesEventos() {	
		...
		jQuery(document).ready(function() {
			...
			funcionGeneralEventoJQuery.setImagenCerrarSesionClic("opcion","seguridad","",opcion_funcion1,opcion_webcontrol1);			

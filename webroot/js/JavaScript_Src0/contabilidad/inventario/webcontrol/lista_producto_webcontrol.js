...
class lista_producto_webcontrol extends lista_producto_webcontrol_add {
	...
	actualizarVariablesPagina(lista_producto_control) {
		...
		if(lista_producto_control.action=="index" || lista_producto_control.action=="load") {
		...
		} else if(lista_producto_control.action=="recargarFomularioGeneral") {
			...
			alert(lista_producto_control.action + " Revisar Manejo");
			
			if(lista_producto_control.bitEsAbrirVentanaAuxiliarUrl==true){
			...
			if(lista_producto_control.bitEsEjecutarFuncionJavaScript==true){
				this.actualizarPaginaEjecutarJavaScript(lista_producto_control);
			}
			...
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarGeneralAjaxWebPart==true) {		
				this.actualizarPaginaCargaGeneral(lista_producto_control);	
			}
			
			if(lista_producto_control.paramDivSeccion.bitDivEsCargarAjaxWebPart==true) {
				this.actualizarPaginaTablaDatos(lista_producto_control);
			}
			
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarMostrarAjaxWebPart==true) {		
			...
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarMostrarSoloBusquedasAjaxWebPart==true) {		
				this.actualizarPaginaAreaBusquedas(lista_producto_control);	
			}
			
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarMostrarSoloMantenimientosAjaxWebPart==true) {		
			...
			if(lista_producto_control.paramDivSeccion.bitDivEsCargarMantenimientosAjaxWebPart==true) {				
				this.actualizarPaginaTablaFilaActual(lista_producto_control);			
			}
			
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarReporteAuxiliarAjaxWebPart==true) {				
				this.actualizarPaginaImprimir(lista_producto_control);
			}
			
			
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarMostrarBusquedasAjaxWebPart==true) {
				this.actualizarPaginaAreaAuxiliarBusquedas(lista_producto_control);
			}
			
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarResumenAjaxWebPart==true) {						
				this.actualizarPaginaAreaAuxiliarResumen(lista_producto_control);
			}				
			
			if(lista_producto_control.paramDivSeccion.bitDivsEsCargarAccionesRelacionesAjaxWebPart==true) {						
				this.actualizarPaginaAreaAuxiliarAccionesRelaciones(lista_producto_control);
			}
			
			if(lista_producto_control.usuarioActual!=null && lista_producto_control.usuarioActual.field_strUserName!=null &&
			lista_producto_control.usuarioActual.field_strUserName!=undefined ) {
				
				this.actualizarPaginaUsuarioInvitado(lista_producto_control);			
			}
		}
...
//Funciones Auxiliares
	...
	actualizarPaginaTablaDatosAuxiliar(lista_producto_control) {
		if(lista_producto_funcion1.esPaginaForm()==true) {
	...
	actualizarPaginaEjecutarJavaScript(lista_producto_control) {
		eval(lista_producto_control.strFuncionJs);
	}
	
	actualizarPaginaMensajePantalla(lista_producto_control, mostrar) {
		
		if(mostrar==true) {
			lista_producto_funcion1.resaltarRestaurarDivsPagina(false);
			
			if(jQuery("#ParamsPostAccion-chbPostAccionSinCerrar").prop('checked')==false
				&& jQuery("#ParamsPostAccion-chbPostAccionNuevo").prop('checked')==false) {		
					
				lista_producto_funcion1.resaltarRestaurarDivMantenimiento(false);
			}			
	...
	actualizarPaginaImprimir(lista_producto_control) {
	
		funcionGeneral.printWebPartPage("lista_producto",lista_producto_control.htmlTablaReporteAuxiliars);			
			
		//jQuery("#divTablaDatosReporteAuxiliarlista_productosAjaxWebPart").html(lista_producto_control.htmlTablaReporteAuxiliars);						
		//funcionGeneral.printWebPartPage("lista_producto",jQuery("#divTablaDatosReporteAuxiliarlista_productosAjaxWebPart").html());			
	}
...
/*---------------------------------- AREA:AUXILIARES ---------------------------*/
	...
	registrarAccionesEventos() {	
		...
		jQuery(document).ready(function() {
			...
			funcionGeneralEventoJQuery.setImagenCerrarSesionClic("lista_producto","inventario","",lista_producto_funcion1,lista_producto_webcontrol1);			

...
//var termino_pago_clienteFuncionGeneral= function () {

class termino_pago_cliente_funcion {
...
//---------- Clic-Seleccionar ----------
	...
	seleccionarMostrarDivAccionesRelacionesActualOnComplete() {
		funcionGeneral.seleccionarMostrarDivAccionesRelacionesActualOnComplete("termino_pago_cliente",this);
	}
...
//------------- Clic-Cancelar -------------------
	...
	cancelarControles() {
		...
		jQuery('cmbtermino_pago_clienteid_cuenta_corriente').val("");
		...
		funcionGeneral.setEmptyControl(document.frmMantenimientotermino_pago_cliente.txtcuenta_corriente_codigo);	
	}
...
/*---------------------------------- AREA:PROCESAR ------------------------*/
	...
	procesarInicioProceso() {		
		funcionGeneral.procesarInicioProceso(termino_pago_cliente_constante1.STR_RELATIVE_PATH);				
	}
	
	procesarFinalizacionProceso() {
		funcionGeneralJQuery.procesarFinalizacionProceso(termino_pago_cliente_constante1.STR_RELATIVE_PATH,"termino_pago_cliente");		
	}	
	
	procesarInicioProcesoSimple() {		
		funcionGeneral.procesarInicioProcesoSimple(termino_pago_cliente_constante1.STR_RELATIVE_PATH);				
	}
	
	procesarFinalizacionProcesoSimple() {
		funcionGeneralJQuery.procesarFinalizacionProcesoSimple(termino_pago_cliente_constante1.STR_RELATIVE_PATH,"termino_pago_cliente");
	}

//------------- Procesar-Link -------------------

	procesarFinalizacionProcesoAbrirLink() {
		funcionGeneralJQuery.procesarFinalizacionProcesoAbrirLink(termino_pago_cliente_constante1.STR_RELATIVE_PATH,"termino_pago_cliente");		
	}
	
	procesarFinalizacionProcesoAbrirLinkParametros(strTipo,strLink) {
		funcionGeneralJQuery.procesarFinalizacionProcesoAbrirLinkParametros(strTipo,strLink,"termino_pago_cliente");
	}
...
/*---------------------------------- AREA:FORMULARIO ----------------------*/
	...
	esPaginaForm() {
		...
		if(termino_pago_cliente_constante1.BIT_CON_PAGINA_FORM==true && termino_pago_cliente_constante1.BIT_ES_PAGINA_FORM==true) {
			bitRetorno =true;
		}
		...
		*/
		...
		if(termino_pago_cliente_constante1.STR_SUFIJO=="") {
			...
			arrRules= {
				...
				"form_termino_pago_cliente-id_cuenta_corriente": {
					digits:true
				...
				"form_termino_pago_cliente-cuenta_corriente_codigo": {
					maxlength:40
					,regexpStringMethod:true
				}
				},
				...
				messages: {
					...
					"form_termino_pago_cliente-id_cuenta_corriente": ""+constantes.STR_MENSAJE_ENTERO_INCORRECTO,
					...
					"form_termino_pago_cliente-cuenta_corriente_codigo": ""+constantes.STR_MENSAJE_TEXTO_INCORRECTO
				}		
			};	
...
/*---------------------------------- AREA:AUXILIAR ---------------------------*/
	...
	resaltarRestaurarDivMantenimiento(bitEsResaltar) {
		funcionGeneralJQuery.resaltarRestaurarDivMantenimiento(bitEsResaltar,"termino_pago_cliente");
	}
	...
	habilitarDeshabilitarControles(bitEsHabilitar) {
		if(bitEsHabilitar==true) {
			...
			funcionGeneral.setReadOnlyControl(document.frmMantenimientotermino_pago_cliente.txtcuenta_corriente_codigo,false);
		} else {
			...
			funcionGeneral.setReadOnlyControl(document.frmMantenimientotermino_pago_cliente.txtcuenta_corriente_codigo,true);
		}	
	}
	...
	resaltarRestaurarDivMensajePopup(bitEsResaltar) {
		funcionGeneralJQuery.resaltarRestaurarDivMensajePopup(bitEsResaltar,"termino_pago_cliente");
	}
	
	resaltarRestaurarDivsPagina(bitEsResaltar) {
		funcionGeneral.resaltarRestaurarDivsPagina(bitEsResaltar,"termino_pago_cliente");
	}
		...
		if(maxima_fila!=null && maxima_fila > 0) {
			for (var i = 1; i <= maxima_fila; i++) { 
				...
				*/
				...
				if(termino_pago_cliente_control.idcuenta_corrienteDefaultForeignKey!=null && termino_pago_cliente_control.idcuenta_corrienteDefaultForeignKey>-1) {
					idActual=termino_pago_cliente_control.idcuenta_corrienteDefaultForeignKey;
				}

				if(jQuery("#"+control_name_id).val()<0) {
					termino_pago_cliente_webcontrol1.cargarComboEditarTablacuenta_corrienteFK(termino_pago_cliente_control,control_name,idActual,false);
				}
				...
				jQuery("#chb_"+control_name).change(function(){
					...
					if(jQuery(this).prop('checked')==true) {
						//alert("FK_ID_ACTUAL="+idActual);
						termino_pago_cliente_webcontrol1.cargarComboEditarTablacuenta_corrienteFK(termino_pago_cliente_control,control_name,idActual,true);
					}
				});

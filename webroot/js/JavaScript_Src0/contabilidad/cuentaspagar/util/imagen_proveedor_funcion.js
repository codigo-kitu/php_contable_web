...
//var imagen_proveedorFuncionGeneral= function () {

class imagen_proveedor_funcion {
...
//---------- Clic-Buscar-Auxiliar ----------
	...
	procesarInicioBusqueda() {
		funcionGeneral.procesarInicioBusquedaPrincipal(imagen_proveedor_constante1.STR_RELATIVE_PATH,"imagen_proveedor");		
	}
	
	procesarFinalizacionBusqueda() {
		funcionGeneral.procesarFinalizacionBusquedaPrincipal(imagen_proveedor_constante1.STR_RELATIVE_PATH,imagen_proveedor_constante1.STR_NOMBRE_OPCION,"imagen_proveedor");		
	}	
...
//---------- Clic-Reporte ----------------
	...
	generarReporteFinalizacion() {
		funcionGeneral.generarReporteFinalizacion(imagen_proveedor_constante1.STR_RELATIVE_PATH,imagen_proveedor_constante1.STR_NOMBRE_OPCION);
	}
...
/*---------------------------------- AREA:PROCESAR ------------------------*/
	...
	procesarInicioProceso() {		
		funcionGeneral.procesarInicioProceso(imagen_proveedor_constante1.STR_RELATIVE_PATH);				
	}
	
	procesarFinalizacionProceso() {
		funcionGeneralJQuery.procesarFinalizacionProceso(imagen_proveedor_constante1.STR_RELATIVE_PATH,"imagen_proveedor");		
	}	
	
	procesarInicioProcesoSimple() {		
		funcionGeneral.procesarInicioProcesoSimple(imagen_proveedor_constante1.STR_RELATIVE_PATH);				
	}
	
	procesarFinalizacionProcesoSimple() {
		funcionGeneralJQuery.procesarFinalizacionProcesoSimple(imagen_proveedor_constante1.STR_RELATIVE_PATH,"imagen_proveedor");
	}

//------------- Procesar-Link -------------------

	procesarFinalizacionProcesoAbrirLink() {
		funcionGeneralJQuery.procesarFinalizacionProcesoAbrirLink(imagen_proveedor_constante1.STR_RELATIVE_PATH,"imagen_proveedor");		
	}
	
	procesarFinalizacionProcesoAbrirLinkParametros(strTipo,strLink) {
		funcionGeneralJQuery.procesarFinalizacionProcesoAbrirLinkParametros(strTipo,strLink,"imagen_proveedor");
	}
...
/*---------------------------------- AREA:FORMULARIO ----------------------*/
	...
	esPaginaForm() {
		...
		if(imagen_proveedor_constante1.BIT_CON_PAGINA_FORM==true && imagen_proveedor_constante1.BIT_ES_PAGINA_FORM==true) {
			bitRetorno =true;
		}
...
/*---------------------------------- AREA:AUXILIAR ---------------------------*/
	...
	resaltarRestaurarDivMantenimiento(bitEsResaltar) {
		funcionGeneralJQuery.resaltarRestaurarDivMantenimiento(bitEsResaltar,"imagen_proveedor");
	}
	...
	resaltarRestaurarDivMensajePopup(bitEsResaltar) {
		funcionGeneralJQuery.resaltarRestaurarDivMensajePopup(bitEsResaltar,"imagen_proveedor");
	}
	
	resaltarRestaurarDivsPagina(bitEsResaltar) {
		funcionGeneral.resaltarRestaurarDivsPagina(bitEsResaltar,"imagen_proveedor");
	}
...
//------------- Tabla-Validacion -------------------
	...
	registrarControlesTableValidacionEdition(imagen_proveedor_control,imagen_proveedor_webcontrol1,imagen_proveedor_constante1) {
		
		var strSuf=imagen_proveedor_constante1.STR_SUFIJO;
		...
		if(maxima_fila!=null && maxima_fila > 0) {
			for (var i = 1; i <= maxima_fila; i++) { 
				...
				*/
				...
				if(imagen_proveedor_control.idproveedorDefaultForeignKey!=null && imagen_proveedor_control.idproveedorDefaultForeignKey>-1) {
					idActual=imagen_proveedor_control.idproveedorDefaultForeignKey;
				}

				if(jQuery("#"+control_name_id).val()<0) {
					imagen_proveedor_webcontrol1.cargarComboEditarTablaproveedorFK(imagen_proveedor_control,control_name,idActual,false);
				}
				...
				jQuery("#chb_"+control_name).change(function(){
					...
					if(jQuery(this).prop('checked')==true) {
						...
						imagen_proveedor_webcontrol1.cargarComboEditarTablaproveedorFK(imagen_proveedor_control,control_name,idActual,true);
					}
				});
		...
		jQuery("#frmTablaDatosimagen_proveedor").validate(json_rules);

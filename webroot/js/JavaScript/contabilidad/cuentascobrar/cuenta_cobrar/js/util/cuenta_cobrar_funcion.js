...
/*---------- Clic-Seleccionar ----------*/
	...
	seleccionarMostrarDivAccionesRelacionesActualOnComplete() { 
		funcionGeneral.seleccionarMostrarDivAccionesRelacionesActualOnComplete("cuenta_cobrar",this,cuenta_cobrar_constante1); 
	}
/*---------- Clic-Reporte ----------------*/
		...
		if(maxima_fila!=null && maxima_fila > 0) {
			for (var i = 1; i <= maxima_fila; i++) { 
				...
				*/
				...
				if(cuenta_cobrar_control.idtermino_pago_clienteDefaultForeignKey!=null && cuenta_cobrar_control.idtermino_pago_clienteDefaultForeignKey>-1) {
					idActual=cuenta_cobrar_control.idtermino_pago_clienteDefaultForeignKey;
				}

				if(jQuery("#"+control_name_id).val()<0) {
					cuenta_cobrar_webcontrol1.cargarComboEditarTablatermino_pago_clienteFK(cuenta_cobrar_control,control_name,idActual,false);
				}
				...
				jQuery("#chb_"+control_name).change(function(){
					...
					if(jQuery(this).prop('checked')==true) {
						//alert("FK_ID_ACTUAL="+idActual);
						cuenta_cobrar_webcontrol1.cargarComboEditarTablatermino_pago_clienteFK(cuenta_cobrar_control,control_name,idActual,true);
					}
				});

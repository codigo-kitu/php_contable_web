...
import {estimado_funcion,estimado_funcion1} from '../util/estimado_form_funcion.js';
...
class estimado_webcontrol extends GeneralEntityWebControl {
	...
	actualizarVariablesPagina(estimado_control) {
		...
		} else if(estimado_control.action=="recargarFormularioGeneral") {
...
//Funciones Manejadores Accion
	...
	actualizarVariablesPaginaAccionSeleccionarActualPaginaForm(estimado_control) {
		...
		this.seleccionarActualPaginaFormularioAdd(estimado_control);
	...
	*/
	...
	actualizarCamposFormulario(estimado_control) {
		...
		jQuery("#form"+estimado_constante1.STR_SUFIJO+"-created_at").val(estimado_control.estimadoActual.versionRow);
		jQuery("#form"+estimado_constante1.STR_SUFIJO+"-updated_at").val(estimado_control.estimadoActual.versionRow);
		...
		if(estimado_control.estimadoActual.id_moneda!=null && estimado_control.estimadoActual.id_moneda>-1){
			if(jQuery("#form"+estimado_constante1.STR_SUFIJO+"-id_moneda").val() != estimado_control.estimadoActual.id_moneda) {
				jQuery("#form"+estimado_constante1.STR_SUFIJO+"-id_moneda").val(estimado_control.estimadoActual.id_moneda).trigger("change");
			}
		} else { 
			//jQuery("#form"+estimado_constante1.STR_SUFIJO+"-id_moneda").select2("val", null);
			if(jQuery("#form"+estimado_constante1.STR_SUFIJO+"-id_moneda").val() != constantes.INT_VALOR_ESCOJA_OPCION) {
				jQuery("#form"+estimado_constante1.STR_SUFIJO+"-id_moneda").val(constantes.INT_VALOR_ESCOJA_OPCION).trigger("change");
			}
		}
		jQuery("#form"+estimado_constante1.STR_SUFIJO+"-cotizacion").val(estimado_control.estimadoActual.cotizacion);
		...
		jQuery("#form"+estimado_constante1.STR_SUFIJO+"-genero_factura").prop('checked',estimado_control.estimadoActual.genero_factura);
	...
	actualizarSpanMensajesCampos(estimado_control) {
		...
		jQuery("#spanstrMensajecreated_at").text(estimado_control.strMensajecreated_at);		
		jQuery("#spanstrMensajeupdated_at").text(estimado_control.strMensajeupdated_at);		
		...
		jQuery("#spanstrMensajeid_moneda").text(estimado_control.strMensajeid_moneda);		
		jQuery("#spanstrMensajecotizacion").text(estimado_control.strMensajecotizacion);		
